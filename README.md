...
replayt logs the request, response metadata, and validated structured output as explicit run events. On success, `structured_output` repeats `usage`, `latency_ms`, and `finish_reason` (and optional provider ids) from the same completion so JSONL-only pipelines can attribute tokens to a schema name without joining the prior `llm_response` line.
