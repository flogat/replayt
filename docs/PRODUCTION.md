...
This page covers one finite run per process, log handling, and exit semantics. For scope boundaries, see [`SCOPE.md`](SCOPE.md).
...
Use a **Job** that runs once and exits. Mount a **PersistentVolumeClaim** (or use an object-store upload step after the Job finishes) for `.replayt` logs if you need retention across Pod restarts.
...
- Choose **`LogMode`** explicitly in Python (`LogMode.redacted`, `structured_only`, or full) or via CLI `--log-mode`. Use **`redacted`** when prompts or payloads may contain PII; see [`RUN_LOG_SCHEMA.md`](RUN_LOG_SCHEMA.md) for what each event records.
...
- Use one log directory (or SQLite file) per tenant, for example `.replayt/runs/customer_a/` on an encrypted volume if required. See the multi-tenant row in [`SCOPE.md`](SCOPE.md).
