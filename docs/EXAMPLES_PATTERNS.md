...
**Approach:** Pin contracts with **`replayt contract TARGET --check path/to/workflow.contract.json`** in CI. At runtime, read **`REPLAYT_WORKFLOW_CONTRACT_SHA256`** (and optional name/version) inside **`run_hook`** / **`resume_hook`** and compare to your allowlist file or CMDB query; exit non-zero to block before new JSONL events. The same three env vars are also injected into **`export_hook`**, **`seal_hook`**, and **`verify_seal_hook`** (parsed from **`run_started`**; **`export_hook`** also receives **`REPLAYT_TARGET`** when you pass **`--target`** on export) so archival and verify steps can reuse one allowlist script. When **`run_started`** recorded **`run_metadata`**, **`tags`**, **`experiment`**, or non-empty **`workflow_meta`** (the **`Workflow.meta`** bag without **`llm_defaults`**), those hooks also receive **`REPLAYT_RUN_METADATA_JSON`**, **`REPLAYT_RUN_TAGS_JSON`**, **`REPLAYT_RUN_EXPERIMENT_JSON`**, and optional **`REPLAYT_WORKFLOW_META_JSON`** (same strings as **`run_hook`**) so tier, ticket, or package-id policy can run without **`jq`**. When the line is readable, optional **`REPLAYT_RUN_STARTED_RUNTIME_JSON`** carries the same **`run_started.runtime`** object as canonical sorted JSON (or **`{}`** when **`runtime`** was omitted or null) so gates can read **`engine.log_mode`**, **`llm.model`**, and nested contract fingerprints without **`jq`**. Optional **`REPLAYT_RUN_STARTED_INITIAL_STATE`** carries **`run_started.initial_state`** as a plain string (or **`""`** when that key is absent on a readable payload; **`run_hook`** on a brand-new run uses the loaded workflow's initial state) so allowlists can reject unexpected entry states without **`jq`**. To detect drift against the original run, parse the first **`run_started`** line in the run's JSONL and compare its **`runtime.workflow.contract_sha256`** to the hook env (or to your allowlist).
...
#### Sub-pattern: OpenAI `tools=` from `ctx.tools`
...
**Approach:** Register functions on `ctx.tools` as usual, pass `tools=ctx.tools.openai_chat_tools()` to `chat.completions.create`, then run the returned `tool_calls` through `ctx.tools.apply_openai_chat_tool_calls([tc.model_dump() for tc in msg.tool_calls])` (or hand-built dicts in tests). That parses `function.arguments` and invokes `ctx.tools.call` in list order. When the model returns a non-empty string `id` on each tool call, replayt copies it to optional `tool_call_id` on the paired `tool_call` / `tool_result` JSONL lines for multi-turn message stitching. For the next completion in the same step, build `role: "tool"` rows with `ctx.tools.openai_chat_tool_result_messages(tcs, tool_results)` (parallel to `apply_openai_chat_tool_calls` return values); each vendor `tool_calls` entry must keep its `id` so OpenAI accepts `tool_call_id`. This stays **composition**: there is no tool routing on `LLMBridge`.
...
def sdk_tools(ctx):
    ...
    r = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": "Call lookup for hats."}],
        tools=ctx.tools.openai_chat_tools(),
        tool_choice="auto",
    )
    msg = r.choices[0].message
    tcs = [tc.model_dump() for tc in (msg.tool_calls or [])]
    tool_returns = ctx.tools.apply_openai_chat_tool_calls(tcs)
    tool_msgs = ctx.tools.openai_chat_tool_result_messages(tcs, tool_returns)
    _ = tool_msgs  # append assistant message + tool_msgs to messages for a second create() if needed
    return "done"
```

#### Sub-pattern: Anthropic `tools=` / `tool_use` from `ctx.tools`

**Scenario:** You use `anthropic.Anthropic().messages.create` with native `tools` and `tool_use` blocks (not the OpenAI-shaped chat completions API).

**Approach:** Register handlers on `ctx.tools`, pass `tools=ctx.tools.anthropic_messages_tools()` to `messages.create`, then run assistant `content` blocks with `ctx.tools.apply_anthropic_tool_use_blocks(blocks)` (or pass only the `tool_use` dicts). Non-`tool_use` blocks are ignored; results are returned in `tool_use` order. Each `tool_use` block's string `id` (when present) is copied to optional `tool_call_id` on the JSONL lines. Each invocation still goes through `ctx.tools.call`, so JSONL gets the same `tool_call` / `tool_result` events as the OpenAI helper path.
...
**Approach:** Register handlers on `ctx.tools`, pass `toolConfig={"tools": ctx.tools.bedrock_converse_tools()}` (and any required `toolChoice`) to `converse`, then run assistant `output` / `message` `content` with `ctx.tools.apply_bedrock_converse_tool_use_blocks(blocks)`. Blocks without `toolUse` are skipped; results are returned in `toolUse` order. Non-empty string `toolUseId` values are copied to optional `tool_call_id` on the JSONL lines. Each invocation still goes through `ctx.tools.call`, so JSONL matches the OpenAI and Anthropic helper paths.
...
@wf.step("anthropic_sdk_tools")
def anthropic_sdk_tools(ctx):
    @ctx.tools.register
    def lookup(q: str) -> dict:
        """Search the catalog."""
        return {"hits": [q]}
    ...
    msg = client.messages.create(
        ...
        max_tokens=1024,
        messages=[{"role": "user", "content": "Call lookup for hats."}],
        tools=ctx.tools.anthropic_messages_tools(),
    )
    blocks = [b.model_dump() for b in msg.content]
    ctx.tools.apply_anthropic_tool_use_blocks(blocks)
    return "done"
```
...
Use **`ctx.note(...)`** for explicit sub-run breadcrumbs, then narrow a run with **`replayt inspect RUN_ID --event-type step_note`** (repeat the flag for OR) when you want framework-shaped signals without `jq`. For **`tool_call`** lines, use **`replayt inspect RUN_ID --tool TOOL_NAME`** (same OR rules as **`replayt runs --tool`**) or **`--event-type tool_call`** when you need every invocation. To find local runs that validated a particular Pydantic **`schema_name`**, or that tagged a freeform **`ctx.llm.complete_text(..., schema_name="…")`** pair, list with **`replayt runs --structured-schema MyModel`** (repeat for OR); **`replayt stats`** and **`replayt inspect RUN_ID --structured-schema MyModel`** accept the same match rules (**`llm_request`** / **`llm_response`** lines count when they carry **`schema_name`**). When you route **`ctx.llm.with_settings(model=...)`** or gateway aliases across steps, list runs that actually hit a provider id with **`replayt runs --llm-model gpt-4o-mini`** (repeat for OR); **`replayt stats`** and **`replayt inspect RUN_ID --llm-model …`** match on logged **`effective.model`** (legacy logs fall back to top-level **`model`** on the same event types). **`replayt report RUN_ID --llm-model …`**, **`replayt diff RUN_A RUN_B --llm-model …`**, and **`replayt report-diff … --llm-model …`** apply the same slice to structured outputs and token-style summaries (diff keeps full-run states and tool-call counts). When a sandboxed graph hits token limits, **`llm_response`** events record the provider **`finish_reason`**; use **`replayt runs --finish-reason length`** or **`replayt inspect RUN_ID --finish-reason length`** to triage those runs without ad-hoc **`jq`**.
...
2. **SQL over runs with datasette:** Point [datasette](https://datasette.io/) at your SQLite store for a read-only SQL UI:
