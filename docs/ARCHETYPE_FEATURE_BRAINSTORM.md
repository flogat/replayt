...
- **Request:** Extend seal tooling so `replayt verify-seal RUN_ID` reports digest mismatches clearly, names what changed when verification fails, and prints a machine-readable status for audit pipelines that archive or hand off run logs.
- **Fits principles:** yes
- **Scope:** in-core candidate
