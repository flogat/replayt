...
- **Request:** Let operators tag runs with a named redaction policy and record which fields were masked, skipped, or sealed so audit reviewers can tell whether a run used default redaction, stricter PII rules, or encrypted logs.
- **Aligns with principles:** yes
- **Scope:** in-core candidate

### 4. ML / LLM engineer — Logged provider preset diffs
- **Request:** Add reusable LLM presets that still stay explicit by logging the full effective settings delta per step, including provider slug, response format mode, timeout, schema fingerprint, and experiment tags.
- **Aligns with principles:** yes
- **Scope:** in-core candidate

### 5. DevOps / SRE — CI readiness doctor
- **Request:** Add a `replayt doctor --ci` mode that checks writable log paths, expected env vars, SQLite mirror availability, approval-resume hooks, and safe failure messages for containerized or queue-worker deployments.
- **Aligns with principles:** yes
- **Scope:** in-core candidate

### 6. Product engineer — Stakeholder handoff report bundle
- **Request:** Expand report/export output with a compact stakeholder bundle that highlights approvals, final decision, structured outputs, and failure points without exposing raw prompt text by default.
- **Aligns with principles:** yes
- **Scope:** in-core candidate
...
- **Request:** Ship a one-command local happy path for the common case: JSONL logs, optional SQLite mirror, one approval checkpoint, one HTML report export, and an example CI command that fails clearly instead of requiring a long setup pass.
- **Aligns with principles:** yes
- **Scope:** in-core candidate
...
- **Suggested overcome:** Keep replayt as the local `Runner` and put identity, policy, and ticket checks in your wrapper: use the README subsection **Policy hooks, eval-style harnesses, and agent frameworks** in [`README.md`](../README.md) plus **Pattern: approval bridge (local UI)** and **Pattern: webhook / lifecycle callbacks** in [`docs/EXAMPLES_PATTERNS.md`](./EXAMPLES_PATTERNS.md).
...
- **Scope:** compose outside core
- **Principle / scope tension:** This conflicts with **Explicit states over hidden loops**, **Replay is part of the product**, and the **LangChain / LangGraph inside core** row in [`docs/SCOPE.md`](./SCOPE.md); per-token and planner-internal events would bury the explicit replay log.
- **Suggested overcome:** Keep external frameworks inside one replayt step and emit one validated exit shape. The existing tutorial subsection **LangGraph (and similar frameworks) — composition, not core** in [`src/replayt_examples/README.md`](../src/replayt_examples/README.md) and **Pattern: framework in a sandbox step** plus **Pattern: stream inside step, log structured summary** in [`docs/EXAMPLES_PATTERNS.md`](./EXAMPLES_PATTERNS.md) already document the supported shape.
