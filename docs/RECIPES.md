...
**Per-call** - override one step without forking the library: `ctx.llm.with_settings(model=..., temperature=..., top_p=..., timeout_seconds=..., max_tokens=..., provider=..., base_url=..., native_response_format=True, extra_headers={...}, extra_body={...}, experiment={...}, call_label=...)`. Overrides appear under `effective` on `llm_request` / `llm_response` events, and the bridge also emits stable `messages_sha256` / `effective_sha256` fingerprints (plus `schema_sha256` and `schema_name` for `parse(...)`, and optional `schema_name` on `complete_text(..., schema_name=...)` when you label a freeform call) so exact prompt and schema revisions stay comparable in redacted logs. Optional `call_label` tags multiple LLM round trips inside the same handler (also top-level on matching LLM and structured-output lines); use `experiment` for run-level cohort or dataset labels, not as a full eval product; see [`SCOPE.md`](SCOPE.md).
