...
**Per-call** - override one step without forking the library: `ctx.llm.with_settings(model=..., temperature=..., top_p=..., timeout_seconds=..., max_tokens=..., http_retries=..., provider=..., base_url=..., native_response_format=True, response_format={"type": "json_object"}, extra_headers={...}, extra_body={...}, experiment={...}, call_label=...)`. The `response_format` dict is forwarded on `complete_text` (clear defaults with `with_settings(response_format={})`, or skip for one call via `complete_text(..., response_format=None)`). `http_retries` is the count of *extra* POST attempts after the first on retryable HTTP statuses (429, 500, 502, 503, 504), inheriting from `LLMSettings.http_retries` when unset on the bridge; cap is 25. You can also pass `complete_text(..., http_retries=...)` or `parse(..., http_retries=...)` for a single call. Overrides appear under `effective` on `llm_request` / `llm_response` events, including a compact `effective.response_format` when the gateway request used one (not the full embedded schema). The bridge also emits stable `messages_sha256` / `effective_sha256` fingerprints (plus `schema_sha256` and `schema_name` for `parse(...)`, and optional `schema_name` on `complete_text(..., schema_name=...)` when you label a freeform call) so exact prompt and schema revisions stay comparable in redacted logs. Optional `call_label` tags multiple LLM round trips inside the same handler (also top-level on matching LLM and structured-output lines); use `experiment` for run-level cohort or dataset labels, not as a full eval product; see [`SCOPE.md`](SCOPE.md).
