...
The sorted allowlist also appears as **`supported_project_config_keys`** on **`replayt version --format json`** so CI can diff the installed parser against your own docs or generated editor schemas without reading source. The same payload includes sorted **`cli_subcommands`** (top-level **`replayt`** commands registered on the installed Typer app) so wrappers can allowlist argv without scraping **`--help`** text, **`cli_stdio_contract`** (when **`run`**, **`ci`**, **`validate`**, or **`doctor`** may read stdin for JSON inputs, including **`REPLAYT_INPUTS_FILE=-`**), plus **`cli_exit_codes`** (workflow **`run` / `ci` / `resume` / `try`** exit semantics and JSON-mode gates for **`doctor`** and **`validate`**) so pipelines can branch without rereading prose docs.
...
| `seal_hook` | Optional argv list for a subprocess run after the JSONL is read and digests are computed but **before** `replayt seal` writes the manifest (policy gate for standalone seals; tarball exports still use `export_hook`). replayt passes `REPLAYT_RUN_ID`, `REPLAYT_LOG_DIR`, `REPLAYT_SEAL_JSONL`, `REPLAYT_SEAL_OUT`, `REPLAYT_SEAL_LINE_COUNT`, and `REPLAYT_WORKFLOW_CONTRACT_SHA256` / `REPLAYT_WORKFLOW_NAME` / `REPLAYT_WORKFLOW_VERSION` from the first `run_started` line in that JSONL. Override with env `REPLAYT_SEAL_HOOK` (shell tokenized; see [`CLI.md`](CLI.md)). Successful seals record a compact `policy_hook` object in the seal manifest. Trusted config only, not untrusted user input. |
...
| `verify_seal_hook` | Optional argv list for a subprocess run after **`replayt verify-seal`** confirms digests match but **before** it prints success output (policy / audit gate). replayt passes `REPLAYT_RUN_ID`, `REPLAYT_LOG_DIR`, `REPLAYT_VERIFY_SEAL_MANIFEST`, `REPLAYT_VERIFY_SEAL_JSONL`, `REPLAYT_VERIFY_SEAL_SCHEMA` (manifest `schema` id), `REPLAYT_VERIFY_SEAL_LINE_COUNT`, `REPLAYT_VERIFY_SEAL_FILE_SHA256`, and `REPLAYT_WORKFLOW_CONTRACT_SHA256` / `REPLAYT_WORKFLOW_NAME` / `REPLAYT_WORKFLOW_VERSION` from the first `run_started` line in the verified JSONL. Override with env `REPLAYT_VERIFY_SEAL_HOOK` (shell tokenized; see [`CLI.md`](CLI.md)). Trusted config only, not untrusted user input. |
