...
When set to a non-empty path, **`replayt run`** writes the same **`replayt.ci_run_summary.v1`** JSON artifact as **`replayt ci --summary-json PATH`** (including **`cwd`**, **`replayt_version`**, **`python_version`**, **`python_implementation`**, **`python_cache_tag`**, **`python_executable`**, **`platform`**, **`machine`**, **`filesystem_encoding`**, **`stdout_encoding`**, **`ci_marker_env`**, **`host_clock_utc_offset_minutes`**, and POSIX **`ulimit_nofile_soft`** / **`ulimit_nofile_hard`** when available alongside run outcome fields). `replayt config --format json` exposes the resolved path under **`ci_artifacts.summary_json`**, and `replayt doctor --format json` adds **`ci_summary_json_ready`** so a broken artifact path fails preflight instead of only failing after execution.
...
**`Runner.run(..., store=JSONLStore(...))` in Python** uses the same default (**`posix_new_file_mode=0o600`**) unless you pass **`posix_new_file_mode=None`**. This env var is read only from **`make_store`** in the CLI layer, not from the **`JSONLStore`** constructor by default.
