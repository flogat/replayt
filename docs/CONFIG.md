...
You end up with one config file: the nearest ancestor that defines either `.replaytrc.toml` or `[tool.replayt]`. See [`src/replayt/cli/main.py`](../src/replayt/cli/main.py) for the implementation.
...
The sorted allowlist also appears as **`supported_project_config_keys`** on **`replayt version --format json`** so CI can diff the installed parser against your own docs or generated editor schemas without reading source. That JSON object also includes **`project_config_discovery`** (`replayt.project_config_discovery.v1`): cwd-independent walk rules and `.replaytrc.toml` vs `pyproject.toml` precedence; **`project_config_resolution`** (`replayt.project_config_resolution.v1`): the same discovery applied to the current **`Path.cwd()`** (ordered **`walk`** rows with **`stopped_here`**, the winning config **`path`** / **`kind`**, and **`unknown_keys`** / **`shadowed_sources`** matching **`replayt config --format json`**); **`cli_run_defaults_contract`** (`replayt.cli_run_defaults_contract.v1`): cwd-independent precedence for optional **`TARGET`** on **`replayt run` / `replayt ci` / `replayt validate` / `replayt graph` / `replayt contract`** and for default inputs resolution shared with **`validate`** and **`doctor --target`**; **`project_setting_precedence_contract`** (`replayt.project_setting_precedence_contract.v1`): cwd-independent precedence for other merged settings (default **`log_dir`**, **`log_mode`**, policy hooks and timeouts, **`forbid_log_mode_full`**, LLM defaults, and similar) with an explicit pointer to **`cli_run_defaults_contract`** so TARGET and inputs stay documented in one place; sorted **`cli_subcommands`** (registered top-level Typer commands) for argv allowlists without scraping **`--help`**; **`cli_stdio_contract`** (when **`run`**, **`ci`**, **`validate`**, or **`doctor`** read stdin JSON, including **`REPLAYT_INPUTS_FILE=-`**); **`cli_json_stdout_contract`** (which **`--output` / `--format` / `--json`** flags pick JSON on stdout and map into **`cli_machine_readable_schemas`**); **`cli_exit_codes`** (workflow **`run` / `ci` / `resume` / `try`** exits and JSON-mode gates for **`doctor`** and **`validate`**); sibling **`run_result_status_contract`** (`replayt.run_result_status_contract.v1`: the only **`status`** strings on **`replayt.run_result.v1`**, each with **`typical_exit_code`** matching **`cli_exit_codes.workflow_run`**); sibling **`log_mode_contract`** (`replayt.log_mode_contract.v1`: canonical **`LogMode`** strings, short summaries, **`--log-mode`**, **`log_mode`**, and **`REPLAYT_LOG_MODE`** on policy hooks); sibling **`jsonl_event_types_contract`** (`replayt.jsonl_event_types_contract.v1`: canonical JSONL **`type`** strings for timeline events replayt core emits, with pointers to **`docs/RUN_LOG_SCHEMA.md`**); **`operational_paths`** (absolute **`cwd`**, **`effective_log_dir`**, **`step_summary`**, env-only **`ci_artifact_paths`**, plus **`ci_artifact_readiness`** / **`ci_artifact_readiness_ok`** using the same writable-path rules as **`replayt doctor`** for env-driven JUnit / summary / step-summary sinks) for pipelines that branch or print paths without rereading prose docs; **`python.implementation`** (**`name`**, dotted **`version`**, optional **`cache_tag`**) and **`platform_machine`** for CPython vs PyPy and architecture gates; and **`distribution_metadata`** (`replayt.distribution_metadata.v1`: installed wheel **`Version`**, **`Requires-Python`**, optional **`Summary`** / **`License`**, sorted **`project_urls`** from **`Project-URL`** headers, or **`ok: false`** with nulls for those fields when no distribution record exists) so matrix jobs can assert Python support and package provenance without parsing **`pyproject.toml`** in the consumer repo.
...
| `log_mode` | Default log mode: `redacted`, `structured_only`, or `full` (canonical strings and summaries: **`log_mode_contract`** on **`replayt version --format json`**, schema **`replayt.log_mode_contract.v1`**). |
...
| `target` | Default workflow target when you omit the **`TARGET`** positional argument on **`replayt run`**, **`replayt ci`**, **`replayt validate`**, **`replayt graph`**, or **`replayt contract`**: a `module:variable` string or a path to `workflow.py` / YAML (same forms as the CLI). Env **`REPLAYT_TARGET`** overrides this when set. **`replayt resume`** still requires **`TARGET`** on argv. **`replayt doctor --target`** uses only the flag value (no fallback from **`REPLAYT_TARGET`** / project **`target`** when the flag is omitted). |
...
| `run_hook` | Optional argv list for a subprocess run before `replayt run` / `replayt ci` starts execution. replayt passes `REPLAYT_POLICY_HOOK_NAME` (`run_hook`), `REPLAYT_TARGET`, `REPLAYT_RUN_ID`, `REPLAYT_RUN_MODE`, `REPLAYT_LOG_DIR`, `REPLAYT_RUN_JSONL` (resolved path to `<run_id>.jsonl` under that log root; the file may not exist yet on a brand-new run), `REPLAYT_LOG_MODE`, `REPLAYT_FORBID_LOG_MODE_FULL` (`1` or `0`), `REPLAYT_REDACT_KEYS_JSON` (sorted JSON array of configured redact field names, never values), `REPLAYT_REPLAYT_VERSION` (installed **replayt** version string), `REPLAYT_DRY_RUN`, optional `REPLAYT_SQLITE`, `REPLAYT_WORKFLOW_CONTRACT_SHA256`, `REPLAYT_WORKFLOW_NAME`, and `REPLAYT_WORKFLOW_VERSION` (from the resolved target's `Workflow.contract()`), optional `REPLAYT_WORKFLOW_ENTRY_PATH` (absolute path to the workflow `.py` / YAML file or the imported module's `__file__` when replayt can resolve it; omitted otherwise), optional `REPLAYT_RUN_STARTED_TS` (ISO 8601 UTC `ts` from the first `run_started` line in the existing JSONL when `--resume` is used and that envelope field is a non-empty string; omitted on brand-new runs before `run_started` exists), optional `REPLAYT_RUN_STARTED_RUNTIME_JSON` (canonical sorted JSON of that line's `runtime`; `{}` when `runtime` is absent or null; omitted on brand-new runs the same way as `REPLAYT_RUN_STARTED_TS`), optional `REPLAYT_RUN_STARTED_INITIAL_STATE` (plain string matching `run_started.initial_state`, or `""` when that key is absent on a readable `run_started` payload; on brand-new runs from the loaded workflow's initial state when set; omitted on `--resume` when `run_started` is unreadable and when a brand-new run's workflow has no initial state), and normalized JSON object strings in `REPLAYT_RUN_INPUTS_JSON`, `REPLAYT_RUN_TAGS_JSON`, `REPLAYT_RUN_METADATA_JSON`, and `REPLAYT_RUN_EXPERIMENT_JSON` when those values are present, plus optional `REPLAYT_WORKFLOW_META_JSON` when `Workflow.meta` (excluding `llm_defaults`) is non-empty (the same object emitted as `run_started.workflow_meta`). Override with env `REPLAYT_RUN_HOOK` (shell tokenized; see [`CLI.md`](CLI.md)). Successful runs record a compact breadcrumb under `run_started.runtime.policy_hooks.run`. Treat like shell commands: trusted config only, not untrusted user input. |
...
| `resume_hook` | Optional argv list for a subprocess run before `replayt resume` appends `approval_resolved` (policy gate). replayt passes `REPLAYT_POLICY_HOOK_NAME` (`resume_hook`), `REPLAYT_TARGET`, `REPLAYT_RUN_ID`, `REPLAYT_LOG_DIR` (resolved log root, same as `run_hook`), `REPLAYT_RUN_JSONL` (resolved path to `<run_id>.jsonl`), `REPLAYT_APPROVAL_ID`, `REPLAYT_REJECT`, `REPLAYT_LOG_MODE`, `REPLAYT_FORBID_LOG_MODE_FULL`, `REPLAYT_REDACT_KEYS_JSON` (effective resume-time logging contract), `REPLAYT_REPLAYT_VERSION` (installed **replayt** version string), the same `REPLAYT_WORKFLOW_CONTRACT_SHA256` / `REPLAYT_WORKFLOW_NAME` / `REPLAYT_WORKFLOW_VERSION` env vars as `run_hook`, optional `REPLAYT_WORKFLOW_ENTRY_PATH` (same resolution rules as `run_hook`), optional `REPLAYT_RUN_STARTED_TS` (ISO 8601 UTC `ts` from the first `run_started` line when that envelope field is a non-empty string), optional `REPLAYT_RUN_INPUTS_JSON` when the first `run_started` line has a readable `inputs` object (canonical sorted JSON; `{}` when `inputs` is absent or null), optional `REPLAYT_RUN_STARTED_RUNTIME_JSON` when that line has a readable `runtime` object (canonical sorted JSON; `{}` when `runtime` is absent or null), optional `REPLAYT_RUN_STARTED_INITIAL_STATE` (plain string from `run_started.initial_state`, or `""` when that key is absent on a readable payload; omitted when `run_started` is missing or unreadable), and optional `REPLAYT_RUN_METADATA_JSON` / `REPLAYT_RUN_TAGS_JSON` / `REPLAYT_RUN_EXPERIMENT_JSON` / `REPLAYT_WORKFLOW_META_JSON` when those objects exist on that line (same JSON strings as `run_hook`; meta is `workflow_meta`). Example: `["python", "scripts/check_resume.py"]`. Override with env `REPLAYT_RESUME_HOOK` (shell tokenized; see [`CLI.md`](CLI.md)). Successful resumes record a compact breadcrumb under `approval_resolved.policy_hook`. Treat like shell commands: trusted config only, not untrusted user input. |
...
| `export_hook` | Optional argv list for a subprocess run before `replayt export-run` or `replayt bundle-export` writes the archive (policy / DLP gate). replayt passes `REPLAYT_POLICY_HOOK_NAME` (`export_hook`), `REPLAYT_RUN_ID`, `REPLAYT_EXPORT_KIND` (`export_run` or `bundle_export`), `REPLAYT_LOG_DIR`, `REPLAYT_LOG_MODE`, `REPLAYT_FORBID_LOG_MODE_FULL`, `REPLAYT_REDACT_KEYS_JSON`, `REPLAYT_REPLAYT_VERSION` (installed **replayt** version string), `REPLAYT_EXPORT_MODE`, `REPLAYT_EXPORT_OUT`, `REPLAYT_EXPORT_SEAL`, `REPLAYT_EXPORT_EVENT_COUNT`, optional `REPLAYT_SQLITE`, `REPLAYT_BUNDLE_REPORT_STYLE` for bundle exports, `REPLAYT_WORKFLOW_CONTRACT_SHA256` / `REPLAYT_WORKFLOW_NAME` / `REPLAYT_WORKFLOW_VERSION` parsed from the run's first `run_started` line (same digest as `run_started.runtime.workflow.contract_sha256`), optional `REPLAYT_RUN_STARTED_TS` (ISO 8601 UTC `ts` from that line's envelope when non-empty), optional `REPLAYT_RUN_INPUTS_JSON` from that line when readable (canonical sorted `inputs`; `{}` when absent or null), optional `REPLAYT_RUN_STARTED_RUNTIME_JSON` from that line when readable (canonical sorted `runtime`; `{}` when absent or null), optional `REPLAYT_RUN_STARTED_INITIAL_STATE` (plain string from `run_started.initial_state`, or `""` when that key is absent on a readable payload; omitted when `run_started` is missing or unreadable), optional `REPLAYT_RUN_METADATA_JSON` / `REPLAYT_RUN_TAGS_JSON` / `REPLAYT_RUN_EXPERIMENT_JSON` / `REPLAYT_WORKFLOW_META_JSON` from that line when present (meta is `workflow_meta`), and when you pass `--target` on the export command, `REPLAYT_TARGET` plus optional `REPLAYT_WORKFLOW_ENTRY_PATH` (same resolution rules as `run_hook`). Override with env `REPLAYT_EXPORT_HOOK` (shell tokenized; see [`CLI.md`](CLI.md)). Successful exports record a compact `policy_hook` object in the export manifest. Trusted config only, not untrusted user input. |
...
| `seal_hook` | Optional argv list for a subprocess run after the JSONL is read and digests are computed but **before** `replayt seal` writes the manifest (policy gate for standalone seals; tarball exports still use `export_hook`). replayt passes `REPLAYT_POLICY_HOOK_NAME` (`seal_hook`), `REPLAYT_RUN_ID`, `REPLAYT_LOG_DIR`, `REPLAYT_LOG_MODE`, `REPLAYT_FORBID_LOG_MODE_FULL`, `REPLAYT_REDACT_KEYS_JSON`, `REPLAYT_REPLAYT_VERSION` (installed **replayt** version string), `REPLAYT_SEAL_JSONL`, `REPLAYT_SEAL_OUT`, `REPLAYT_SEAL_LINE_COUNT`, `REPLAYT_WORKFLOW_CONTRACT_SHA256` / `REPLAYT_WORKFLOW_NAME` / `REPLAYT_WORKFLOW_VERSION` from the first `run_started` line in that JSONL, optional `REPLAYT_RUN_STARTED_TS` (ISO 8601 UTC `ts` from that line's envelope when non-empty), optional `REPLAYT_RUN_INPUTS_JSON` from that line when readable (canonical sorted `inputs`; `{}` when absent or null), optional `REPLAYT_RUN_STARTED_RUNTIME_JSON` from that line when readable (canonical sorted `runtime`; `{}` when absent or null), optional `REPLAYT_RUN_STARTED_INITIAL_STATE` (plain string from `run_started.initial_state`, or `""` when that key is absent on a readable payload; omitted when `run_started` is missing or unreadable), and optional `REPLAYT_RUN_METADATA_JSON` / `REPLAYT_RUN_TAGS_JSON` / `REPLAYT_RUN_EXPERIMENT_JSON` / `REPLAYT_WORKFLOW_META_JSON` from that line when present (meta is `workflow_meta`). Override with env `REPLAYT_SEAL_HOOK` (shell tokenized; see [`CLI.md`](CLI.md)). Successful seals record a compact `policy_hook` object in the seal manifest. Trusted config only, not untrusted user input. |
...
| `verify_seal_hook` | Optional argv list for a subprocess run after **`replayt verify-seal`** confirms digests match but **before** it prints success output (policy / audit gate). replayt passes `REPLAYT_POLICY_HOOK_NAME` (`verify_seal_hook`), `REPLAYT_RUN_ID`, `REPLAYT_LOG_DIR`, `REPLAYT_LOG_MODE`, `REPLAYT_FORBID_LOG_MODE_FULL`, `REPLAYT_REDACT_KEYS_JSON`, `REPLAYT_REPLAYT_VERSION` (installed **replayt** version string), `REPLAYT_VERIFY_SEAL_MANIFEST`, `REPLAYT_VERIFY_SEAL_JSONL`, `REPLAYT_VERIFY_SEAL_SCHEMA` (manifest `schema` id), `REPLAYT_VERIFY_SEAL_LINE_COUNT`, `REPLAYT_VERIFY_SEAL_FILE_SHA256`, `REPLAYT_WORKFLOW_CONTRACT_SHA256` / `REPLAYT_WORKFLOW_NAME` / `REPLAYT_WORKFLOW_VERSION` from the first `run_started` line in the verified JSONL, optional `REPLAYT_RUN_STARTED_TS` (ISO 8601 UTC `ts` from that line's envelope when non-empty), optional `REPLAYT_RUN_INPUTS_JSON` from that line when readable (canonical sorted `inputs`; `{}` when absent or null), optional `REPLAYT_RUN_STARTED_RUNTIME_JSON` from that line when readable (canonical sorted `runtime`; `{}` when absent or null), optional `REPLAYT_RUN_STARTED_INITIAL_STATE` (plain string from `run_started.initial_state`, or `""` when that key is absent on a readable payload; omitted when `run_started` is missing or unreadable), and optional `REPLAYT_RUN_METADATA_JSON` / `REPLAYT_RUN_TAGS_JSON` / `REPLAYT_RUN_EXPERIMENT_JSON` / `REPLAYT_WORKFLOW_META_JSON` from that line when present (meta is `workflow_meta`). Override with env `REPLAYT_VERIFY_SEAL_HOOK` (shell tokenized; see [`CLI.md`](CLI.md)). Trusted config only, not untrusted user input. |
...
| `policy_hook_context_json` | Optional JSON **object** as a UTF-8 string (same shape as `--metadata-json`). When set, and when you omit **`--policy-hook-context-json`** and env **`REPLAYT_POLICY_HOOK_CONTEXT_JSON`**, trusted policy hooks receive that payload as **`REPLAYT_POLICY_HOOK_CONTEXT_JSON`** (sorted keys). It is **not** written to JSONL. Use it for CI correlation ids, pipeline URLs, or wrapper-only hints you do not want on **`run_started.run_metadata`**. |
...
When set to a non-empty string, must be a JSON **object**. replayt canonicalizes it (sorted keys) and injects it into every trusted policy-hook subprocess (`run_hook`, `resume_hook`, `export_hook`, `seal_hook`, `verify_seal_hook`) as **`REPLAYT_POLICY_HOOK_CONTEXT_JSON`**. It is **not** written to JSONL. **`--policy-hook-context-json`** on **`replayt run`**, **`ci`**, **`resume`**, **`export-run`**, **`bundle-export`**, **`seal`**, or **`verify-seal`** overrides this env var; **`[tool.replayt] policy_hook_context_json`** is the fallback when both are unset.
...
When set to a non-empty path, **`replayt run`** writes the same **`replayt.ci_run_summary.v1`** JSON artifact as **`replayt ci --summary-json PATH`** (including **`cwd`**, **`replayt_version`**, **`python_version`**, **`python_implementation`**, **`python_cache_tag`**, **`python_utf8_mode`**, **`python_executable`**, **`platform`**, **`machine`**, **`os_cpu_count`**, **`filesystem_encoding`**, **`stdout_encoding`**, **`ci_marker_env`**, **`host_clock_utc_offset_minutes`**, and POSIX **`ulimit_nofile_soft`** / **`ulimit_nofile_hard`** when available alongside run outcome fields). `replayt config --format json` exposes the resolved path under **`ci_artifacts.summary_json`**, and `replayt doctor --format json` adds **`ci_summary_json_ready`** so a broken artifact path fails preflight instead of only failing after execution.
...
## `REPLAYT_JSONL_POSIX_MODE`

**Unix only.** When the CLI constructs the primary **`JSONLStore`** (and optional **`SQLiteStore`** mirror) via **`make_store`** for **`replayt run`**, **`replayt ci`**, and similar write paths, each **new** run file (`<run_id>.jsonl`) is initialized with **`chmod`** to owner read/write only (**`0600`**) after the first placeholder line is written, so shared hosts do not leave fresh audit logs group- or world-readable by default. When **`--sqlite`** is set and the database file did not exist yet, the same resolved mode is applied to the new SQLite database path and any **`-wal`** / **`-shm`** sidecars SQLite has already created after schema initialization.

Unset or omitted behaves like **`600`**. Set **`inherit`** (or **`umask`**, **`none`**, **`no`**, or empty) to skip **`chmod`** and leave permissions to the process umask. Any other value must be a three- to four-digit **octal** mode (**`660`**, **`0o640`**, …); invalid strings log a warning and fall back to **`0600`**.

**`Runner.run(..., store=JSONLStore(...))` in Python** uses the same default (**`posix_new_file_mode=0o600`**) unless you pass **`posix_new_file_mode=None`**. **`SQLiteStore(..., posix_new_db_file_mode=0o600)`** mirrors that default; pass **`posix_new_db_file_mode=None`** when you construct the mirror yourself and want umask-only semantics. This env var is read only from **`make_store`** in the CLI layer, not from the **`JSONLStore`** / **`SQLiteStore`** constructors by default.
