...
For pipeline branching on **`replayt run` / `ci` / `resume` / `try`** without rereading prose docs, consume **`cli_exit_codes`** from **`replayt version --format json`** (stable alongside **`cli_machine_readable_schemas`**). Subprocess hosts that parse **`replayt.run_result.v1`** on stdout should treat **`status`** as one of **`completed`**, **`failed`**, or **`paused`** only; the same JSON exposes **`run_result_status_contract`** (`replayt.run_result_status_contract.v1`) so **`typical_exit_code`** stays aligned with **`cli_exit_codes.workflow_run`** without reading **`runner.py`**. Policy hooks that branch on logging posture can read **`log_mode_contract`** (`replayt.log_mode_contract.v1`) for the only accepted **`REPLAYT_LOG_MODE`** values and how they relate to **`--log-mode`** / **`[tool.replayt] log_mode`**.

**`replayt.ci_run_summary.v1`** carries **`cwd`**, **`replayt_version`**, **`python_version`**, **`python_implementation`**, **`python_cache_tag`** (PEP 3149 tag from **`sys.implementation`** when present), **`python_executable`**, **`platform`**, **`machine`**, **`filesystem_encoding`**, **`stdout_encoding`**, **`ci_marker_env`** (which common CI env vars are present; **booleans only**), **`host_clock_utc_offset_minutes`**, and (on POSIX) **`ulimit_nofile_soft`** / **`ulimit_nofile_hard`** next to **`run_id`** and **`exit_code`**, so an uploaded **`summary.json`** stays self-describing when you compare flaky jobs across image, interpreter family (CPython vs PyPy), ABI tag, CPU architecture, CI vendor hints, local clock offset, venv, working-directory mistakes, Unicode path behavior, or **`EMFILE`**-style file-descriptor ceilings.
...
On **Unix**, new run **`*.jsonl`** files default to **`0600`** when the CLI builds the store; export **`REPLAYT_JSONL_POSIX_MODE=inherit`** when a **shared group** must read fresh logs without an explicit **`chmod`** step, or pass **`posix_new_file_mode=0o660`** to **`JSONLStore(...)`** when you construct the store for **`Runner(wf, store=...)`** yourself.
