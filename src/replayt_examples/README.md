...
To skip reading this file front to back, run `replayt try --list` (each entry ends with a copy-paste `replayt try --example KEY` line), then run that command or pick another key. Use `replayt try --list --output json` when you want **`replayt.try_examples.v1`**: every example includes a **`cli`** object (`try_offline`, `try_live`, `try_dry_check`, `copy_to_dot`) for scripts and internal docs. Add `--inputs-file my-inputs.json` when you want the same example shape with your own payload. To edit the tutorial code in your repo, run `replayt try --example KEY --copy-to ./my-flow` (writes `workflow.py`, `inputs.example.json`, and `.replaytrc.toml`; use `--force` to replace). The text output prints `doctor --skip-connectivity --target workflow.py`, then bare `replayt run --dry-check` / `replayt run`, with `--dry-run` first when the copied example uses an LLM. When you already have **`workflow.py`** (copy-to, hand-written, or vendored) but **`.gitignore`** never picked up **`.replayt/`** or local env files, run **`replayt init-gitignore`** in that directory. That adds the same lines **`replayt init`** would append, without touching scaffold files. When **`.env.example`** is missing (or you want the same canonical template **`replayt init`** ships), run **`replayt init-env-example`**; it refuses to clobber an existing file unless you pass **`--force`**. replayt still does not auto-load **`.env`**; copy values into **`.env`** or export them in your shell the way your team prefers.
...
An explicit **`TARGET` positional argument always overrides** the default. Check what applies in your shell with **`replayt config --format json`** (`run.default_target`, `run.default_target_source`). **`replayt validate`**, **`replayt graph`**, and **`replayt contract`** use the same **`REPLAYT_TARGET`** / project **`target`** fallback when you omit the positional (like **`replayt run`** / **`replayt ci`**). **`replayt resume`** still requires **`TARGET`** on argv. **`replayt doctor`** only loads a workflow when you pass **`--target …`** (no implicit default from env or project config for that flag).
...
### Beyond core: ad-hoc JSONL views and run_id clipboard

replayt does not rewrite run **`.jsonl`** files into pretty-printed multi-line JSON: seals, **`verify-seal`**, and export digests assume stable NDJSON bytes. For a readable slice, use **`replayt inspect RUN_ID --output json`**, **`replayt replay`**, or pipe a copy of the file through **`jq`**. replayt also does not call OS clipboard APIs: terminals, remote SSH sessions, and CI runners disagree on how pasteboards work. Copy **`run_id=`** from **`replayt run`** or the first column of **`replayt runs`**, or add a **`run_hook`** that prints **`export REPLAYT_RUN_ID=…`** when your team wants a shell-local shortcut.

```bash
replayt inspect "$RUN_ID" --output json | jq .
```
...
replayt does not ship a blessed container image: your registry, Python minor, and whether you need **`replayt[yaml]`** belong in **your** Dockerfile or CI job. A typical pattern is a slim Python base, install the package, then call **`replayt ci`** with the same flags you want everywhere (**`--strict-graph`**, **`--summary-json`**, **`--junit-xml`**) instead of relying on implicit environment detection. After the graph checks pass locally, **`replayt validate your_pkg.workflow:wf --print-ci-one-liner`** prints a POSIX-quoted **`replayt ci … --inputs-json …`** line with the same **`@wf.step(expects=…)`** placeholder union as **`--print-inputs-template`**; adapt **`CMD`** or job **`run:`** from that string, then replace skeleton JSON with **`--inputs-file`** or production JSON.
...
For pipeline branching on **`replayt run` / `ci` / `resume` / `try`** without rereading prose docs, consume **`cli_exit_codes`** from **`replayt version --format json`** (stable alongside **`cli_machine_readable_schemas`**). Subprocess hosts that parse **`replayt.run_result.v1`** on stdout should treat **`status`** as one of **`completed`**, **`failed`**, or **`paused`** only; the same JSON exposes **`run_result_status_contract`** (`replayt.run_result_status_contract.v1`) so **`typical_exit_code`** matches **`cli_exit_codes.workflow_run`** without reading **`runner.py`**. Policy hooks that branch on logging posture can read **`log_mode_contract`** (`replayt.log_mode_contract.v1`) for the only accepted **`REPLAYT_LOG_MODE`** values and how they relate to **`--log-mode`** / **`[tool.replayt] log_mode`**. JSONL analytics and contract tests can read **`jsonl_event_types_contract`** (`replayt.jsonl_event_types_contract.v1`) for the sorted allowlist of envelope **`type`** strings replayt core emits (payload detail stays in **`docs/RUN_LOG_SCHEMA.md`**).

**`replayt.ci_run_summary.v1`** carries **`cwd`**, **`replayt_version`**, **`python_version`**, **`python_implementation`**, **`python_cache_tag`** (PEP 3149 tag from **`sys.implementation`** when present), **`python_utf8_mode`** (**`sys.flags.utf8_mode`**: **1** under **`PYTHONUTF8=1`** / **`-X utf8`**, else **0**), **`python_executable`**, **`platform`**, **`machine`**, **`os_cpu_count`** (**`os.cpu_count()`** or **`null`**), **`filesystem_encoding`**, **`stdout_encoding`**, **`ci_marker_env`** (which common CI env vars are present; **booleans only**), **`host_clock_utc_offset_minutes`**, and (on POSIX) **`ulimit_nofile_soft`** / **`ulimit_nofile_hard`** next to **`run_id`** and **`exit_code`**, so an uploaded **`summary.json`** stays self-describing when you compare flaky jobs across image, interpreter family (CPython vs PyPy), ABI tag, CPU architecture, logical CPU count, UTF-8 mode, CI vendor hints, local clock offset, venv, working-directory mistakes, Unicode path behavior, or **`EMFILE`**-style file-descriptor ceilings.
...
### Beyond core: JSONL envelope `type` checks (CI, not `append_event`)

**`jsonl_event_types_contract`** on **`replayt version --format json`** lists the canonical **`type`** strings replayt core writes to each JSONL line. Core does **not** hard-reject unknown **`type`** values inside **`JSONLStore.append_event`**: approval bridges and other tooling append **`approval_resolved`** (and must match **`docs/RUN_LOG_SCHEMA.md`**) without going through **`Runner`**. When compliance wants a strict diff, compare archived logs to the installed contract in **your** CI step instead of forking storage.

```bash
replayt version --format json | jq -r '.jsonl_event_types_contract.event_types[].value' | sort -u > /tmp/rt-types.txt
grep -v '^[[:space:]]*$' .replayt/runs/some-run.jsonl | jq -r '.type' | sort -u | comm -23 - /tmp/rt-types.txt
```

Do not merge org-specific **`extra_jsonl_event_types`** into **`replayt version --format json`**: that would version a tenant-only surface inside core. Keep foreign audit rows in sidecar files or maintain a private allowlist next to the canonical list above.

### Beyond core: merging `replayt version` and `replayt config` JSON

**`replayt config --format json`** answers what is **effective for the current working directory** (resolved log paths, hooks, default target when env and argv omit it). **`replayt version --format json`** adds cwd-independent contracts such as **`cli_run_defaults_contract`** (`replayt.cli_run_defaults_contract.v1`), which documents the ordered precedence for optional **`TARGET`** on **`replayt run` / `replayt ci` / `replayt validate` / `replayt graph` / `replayt contract`** and for **`resolve_run_inputs_json`** on **`run` / `ci` / `validate` / `doctor --target`**, and **`project_setting_precedence_contract`** (`replayt.project_setting_precedence_contract.v1`), which documents precedence for default **`log_dir`**, **`log_mode`**, policy hooks, LLM defaults, and related merges (it references **`cli_run_defaults_contract`** so TARGET and inputs are not duplicated). Keep those surfaces separate so local shells stay small; when a platform job needs both shapes in one blob, merge in your pipeline:
...
```bash
replayt version --format json > /tmp/rtv.json
replayt config --format json > /tmp/rtc.json
jq -s '.[0].cli_run_defaults_contract as $c | {cli_run_defaults_contract: $c, config_report: .[1]}' /tmp/rtv.json /tmp/rtc.json
```
...
### Beyond core: temp directory probes and `umask` snapshots

Some CI flakes come from a full or **noexec** temp root, or from a surprising **`umask`** on the runner. replayt does **not** embed **`TMPDIR`** / **`TEMP`** writable checks in **`replayt.operational_paths.v1`** or hard-fail **`replayt doctor`** on temp: which env names win and whether a probe should abort the job vary by OS, **Windows** sandboxes, and shared builders. Exercise the directory your stack really uses before **`replayt ci`**:

```bash
python -c "import pathlib, tempfile; d=pathlib.Path(tempfile.gettempdir()); print(d); open(d/'replayt-ci-probe','w').close()"
```

**`umask`** is also session-wide and policy-sensitive; replayt does **not** stamp it into **`replayt.ci_run_summary.v1`**. Emit it from **`run_hook`** or your job wrapper when auditors need proof of inherited file modes:

```bash
umask
```

### Beyond core: GitHub Actions workflow commands and `pip check` beside `replayt doctor`

Some teams want **`replayt ci`** to print GitHub Actions **workflow commands** such as **`::error::`** or **`::notice::`** so failures surface in the job UI. Those strings are **GitHub-specific**; GitLab, Jenkins, and Azure DevOps use different annotation formats, and embedding vendor markers in core would surprise other runners. After **`replayt ci`** exits, read **`replayt.ci_run_summary.v1`** (or **`$?`**) from a thin shell wrapper and emit the commands your platform expects.

Likewise, **`python -m pip check`** (conflicting or broken optional extras) belongs in **your** packaging CI, not **`replayt doctor`**: resolver policy and which extras must be consistent vary by repository.

```bash
SUMMARY=/tmp/replayt-ci-summary.json
STATUS=0
replayt ci mypkg.workflow:wf --summary-json "$SUMMARY" || STATUS=$?
if [ "$STATUS" -ne 0 ] && [ -n "${GITHUB_ACTIONS:-}" ] && command -v jq >/dev/null 2>&1; then
  echo "::error::replayt ci failed status=$(jq -r .status "$SUMMARY") run_id=$(jq -r .run_id "$SUMMARY")"
fi
exit "$STATUS"
```
...
### Beyond core: Unreleased text on `replayt version` JSON and changelog auto-formatters

Some forks want **`replayt version --format json`** to embed **`## Unreleased`** markdown, duplicate **`body_sha256`**, or list bullet digests so one subprocess prints both package metadata and changelog state. The version report is intentionally **cwd-independent** and describes the **installed wheel**; unpublished markdown exists only in your checkout. Run **`python scripts/changelog_unreleased.py --format json`** (with **`--changelog /abs/path/CHANGELOG.md`** when the job cwd is not the repo root) when CI needs the Unreleased body, **`body_sha256`**, **`item_sha256s`**, or parsed **`items`**. Fragment-driven release-note tools (**towncrier**, **scriv**, …) and per-PR **`changelog.d/`** workflows stay in **your** repository; upstream keeps a single reviewed **`CHANGELOG.md`**. Likewise, replayt does **not** ship a script that re-wraps changelog bullets to a fixed column width; wrapping rules, soft breaks in URLs, and archetype-prefix wording vary by team. Use **Prettier**, **mdformat**, **markdownlint**, or a custom formatter in a separate CI job so prose policy stays explicit.

```bash
python scripts/changelog_unreleased.py --format json --changelog CHANGELOG.md | python -c "import json,sys; d=json.load(sys.stdin); print(d['body_sha256'], d['item_sha256s'])"
```

### Beyond core: bundled pip extra installers and venv scaffolding

replayt does **not** run **`pip`**, **`uv`**, or **`python -m venv`** for you from the CLI. Resolver choice, private indexes, and who may change **site-packages** belong in your shell or CI image. When YAML workflows need **PyYAML**, install the extra yourself; errors from **`replayt run workflow.yaml`** already mention **`pip install replayt[yaml]`** when the module is missing. For a fresh checkout, create **`.venv`** with **`python -m venv .venv`**, activate it (see the quickstart for Windows vs POSIX), then **`pip install replayt`** or **`pip install -e ".[dev]"`** when you are contributing. After **`replayt validate`** (with **`TARGET`** on argv or resolved from **`REPLAYT_TARGET`** / project **`target`**) succeeds in text mode, copy the printed **Next:** / **Then:** lines for **`--dry-check`** and a real run. When your steps declare **`@wf.step(..., expects={...})`**, **`replayt validate --print-inputs-template`** (same target resolution) prints a compact JSON skeleton on stdout (placeholders per type; conflicting annotations become **`null`**) so you can redirect to **`inputs.json`** or compare with **`inputs.example.json`** from **`replayt init`**. **`replayt validate --print-run-one-liner`** prints a single **`replayt run … --inputs-json …`** line (POSIX **`shlex`**) with that same skeleton when you want to paste straight into a shell after the graph check passes. **`replayt validate --print-ci-one-liner`** prints **`replayt ci …`** the same way when you are wiring **`jobs.*.steps`** (add **`--summary-json`**, **`--junit-xml`**, or **`--github-summary`** in YAML as needed).

```bash
python -m venv .venv
pip install "replayt[yaml]"
```

### Beyond core: dependency lockfiles and secret sync

Some teams want **`replayt freeze`**, **`replayt deps`**, or a command that turns workflow imports into **`requirements.txt`**. Import graphs and resolver choice (**`pip-tools`**, **`uv lock`**, Poetry) stay in **your** packaging layer; replayt workflows are normal Python modules. Capture pins with **`pip freeze`**, **`uv pip freeze`**, or **`pip-compile`** in CI and commit the lockfile your team already uses. When you only need a copy-paste **`replayt run`** line for a packaged tutorial workflow, **`replayt try --example KEY --print-snippet run`** (or **`run_dry_check`**) prints **`MODULE:VAR`** plus compact **`--inputs-json`** with POSIX **`shlex.quote`** wrapping.

```bash
uv pip freeze > requirements.txt
replayt try --example issue-triage --print-snippet run_dry_check
```

For **`.env`** populated from **1Password**, **Doppler**, or another vault, replayt keeps env provenance explicit: run your vendor CLI or an **`op run --`** wrapper before **`replayt run`**, or set **`REPLAYT_*`** and **`OPENAI_API_KEY`** from the CI job's secret store. **`replayt doctor`** reports what the process can see; it does not pull cloud secrets into the shell for you.

```bash
op run --env-file=.env.template -- replayt run mypkg.workflow:wf --inputs-file inputs.json
```
...
```bash
replayt version --format json | jq -r '.cli_json_stdout_contract.subcommands["inspect"][] | .route_id'
```
...
```bash
replayt version --format json | jq '.cli_json_stdout_contract.trust_profile_machine_flags.inventory_read_only'
```
...
```bash
# Exit codes that can follow a parseable run_result JSON body on stdout (0=completed, 1=failed, 2=paused).
replayt version --format json | jq '.cli_json_stdout_contract.subcommands.run[] | select(.schema_key=="run_result") | .exit_codes_with_json_on_stdout'
```
...
### Beyond core: policy hook output limits and shared log-directory modes

replayt does **not** cap how many bytes a **`run_hook`** or **`resume_hook`** subprocess writes to stdout or stderr, or tee those streams into JSONL: truncation policy, log redaction, and whether to fail the gate on large output belong in **your** wrapper (for example wrap the hook in **`bash -c`** and redirect to **`head -c …`** or a tempfile you scan before the parent sees it).

On **Unix**, new run **`*.jsonl`** files and **new** SQLite mirror files default to **`0600`** when the CLI builds the store; export **`REPLAYT_JSONL_POSIX_MODE=inherit`** when a **shared group** must read fresh logs or the mirror without an explicit **`chmod`** step, or pass **`posix_new_file_mode=0o660`** to **`JSONLStore(...)`** / **`posix_new_db_file_mode=0o660`** to **`SQLiteStore(...)`** when you construct stores for **`Runner(wf, store=...)`** yourself.

```bash
REPLAYT_JSONL_POSIX_MODE=inherit replayt ci "$TARGET"
```
...
```bash
replayt runs --log-dir .replayt/runs --output markdown --style stakeholder > runs-inventory.md
```
...
### Beyond core: temperature grids and implicit parse prompt stuffing

**Hyperparameter grids** (temperature, `top_p`, or other sampling sweeps) are not a **`Runner`** primitive: replayt is a workflow runner, not an eval harness ([`docs/SCOPE.md`](../../docs/SCOPE.md)). Run one trial per **`run_id`** from a **pytest** parametrization, a shell loop, or your job queue; stamp **`--experiment-json`** with the knob under test and use **`ctx.llm.with_settings(temperature=...)`** (or per-call **`parse(..., temperature=...)`**) so each JSONL line stays attributable (**Pattern: batch driver**, **Pattern: golden path test** in [`docs/EXAMPLES_PATTERNS.md`](../../docs/EXAMPLES_PATTERNS.md)). **`ctx.llm.parse`** also does **not** silently merge **`Field(description=...)`**, schema **`examples`**, or **`default`** blobs into its system prompt; that would hide prompt drift from reviewers. Put shape hints in explicit **`messages`** (few-shot user/assistant turns or a system string you own) next to the **`parse`** call.

```bash
for t in 0 0.3 0.7; do
  replayt run mypkg.workflow:wf \
    --inputs-json "{\"temperature\":$t}" \
    --experiment-json "{\"temp_sweep\":$t}"
done
```
...
Share a read-only timeline for review without building a server (add **`--style stakeholder`** or **`support`** to drop **`llm_*`** / **`tool_*`** rows and match **`replayt report`** handoffs). **HTML** embeds the same **Stakeholder CLI handoff** block as **`replayt report`** (inspect, report regen, **`bundle-export`**, **`resume`**, JSON inspect on failure). **Text** stdout stays a compact timeline only; for paste-friendly commands without opening HTML, use **`replayt inspect <run_id> --output markdown`** (default **`--style stakeholder`** matches PM-oriented copy-paste lines; add **`--style support`** for the same **`replayt report` / `replay` / `bundle-export`** support-style commands as **`replayt report --style support`**).
...
**Two-run comparison:** **`replayt report-diff RUN_A RUN_B`** defaults to HTML; add **`--format markdown`** when you need the same stakeholder sections (metadata, failures, structured outputs, approvals) as pasteable text. With **`--style stakeholder`** or **`support`**, the document ends with **Stakeholder CLI handoff** copy-paste lines (regenerate this comparison, **`replayt diff … --output json`**, per-run **`replayt inspect --output markdown --style …`** / matching **`report`** / offline **`replay`**, per-run **`bundle-export`** with matching **`--report-style`** (including **`support-bundle`** filenames when the style is support), JSON **`inspect`** when a side failed, and **`resume`** hints per paused run), using the same handoff pattern as **`replayt report`**. In the terminal, **`replayt diff RUN_A RUN_B --style stakeholder`** (or **`support`**) prints the compact text diff first, then the same handoff block, so you can paste next steps without opening **`report-diff`**. For automation, use **`replayt diff RUN_A RUN_B --output json`** instead of a parallel JSON report-diff schema.

```bash
replayt report-diff <run_a> <run_b> --format markdown --out compare.md
replayt diff <run_a> <run_b> --output json
replayt diff <run_a> <run_b> --style stakeholder
```
...
### Beyond core: OpenAI Assistants threads and smolagents-style tool loops

OpenAI Assistants **threads** and **runs** are long-lived provider state machines with their own polling, tool-output uploads, and file attachments; replayt does not host thread ids or run **`Runner`** against that API surface in core. Hugging Face **smolagents**, Haystack agents, and similar stacks bring their own tool dispatch and implicit replan loops. Keep them inside **one `@wf.step`** (**Pattern: framework in a sandbox step** in **[docs/EXAMPLES_PATTERNS.md](../../docs/EXAMPLES_PATTERNS.md)**): call the vendor client, collapse the outcome you care about into **one** validated object on **`ctx.set`**, then **`return`** the next replayt state. For OpenAI **Chat Completions** tool rounds you orchestrate beside replayt, **`ctx.tools.apply_openai_chat_tool_calls`** plus **`ctx.tools.openai_chat_tool_result_messages`** append aligned **`role: "tool"`** rows (using each vendor **`id`** as **`tool_call_id`**) before the next **`chat.completions.create`**.

```bash
replayt run my_pkg.framework_sandbox:wf --inputs-file inputs.example.json
```

```python
# After msg.tool_calls and apply_openai_chat_tool_calls:
# tool_msgs = ctx.tools.openai_chat_tool_result_messages(tcs, results)
# messages += [{"role": "assistant", "content": msg.content, "tool_calls": tcs}, *tool_msgs]
```

### Beyond core: LangGraph tool nodes and Ollama tool JSON

LangGraph **`ToolNode`** and similar helpers often want to push return dicts straight into shared graph state. replayt does **not** mirror that pattern onto **`RunContext`**: letting a framework mutate **`ctx.set`** keys from tool outputs hides which branch wrote what. Keep the graph **inside one `@wf.step`**, read **`graph.invoke(...)`** (or **`astream_events`** summaries you collapse yourself), validate **one** exit object, then assign **`ctx.set`** / **`return`** explicitly. For **Ollama**, the supported composition path is the **OpenAI-compatible** HTTP surface (for example **`OPENAI_BASE_URL=http://127.0.0.1:11434/v1`** with **`OPENAI_API_KEY=ollama`** or the dummy key your server expects) so you can reuse **`ctx.tools.openai_chat_tools()`** and **`apply_openai_chat_tool_calls`** instead of forking core for native **`/api/chat`** tool JSON.

```bash
# Example: OpenAI client pointed at local Ollama compat (adjust host/port)
export OPENAI_BASE_URL=http://127.0.0.1:11434/v1
export OPENAI_API_KEY=ollama
```

```python
# Inside @wf.step after a LangGraph invoke: map explicitly; do not auto-bind tools to ctx
# out = graph.invoke({"messages": msgs})
# ctx.set("agent_bundle", AgentExit.model_validate(out).model_dump())
```
...
### Audit-only `llm_tags` on LLM lines

**`llm_tags`** are short strings (up to **16** unique tags, **64** characters each after strip) you attach to LLM traffic for **`jq` / DuckDB** filters (prompt pack ids, eval splits, or A/B buckets) without nesting another object under **`experiment`**. Set them in **`Workflow(..., llm_defaults={"llm_tags": ["baseline"]})`**, merge more with **`ctx.llm.with_settings(llm_tags=["ab:high_temp"])`** (union of prior and new tags, then sorted), or pass **`complete_text(..., llm_tags=["nightly"])`** / **`parse(..., llm_tags=[...])`** for one call. They appear on **`payload.effective.llm_tags`** and as top-level **`payload.llm_tags`** on **`llm_request`**, **`llm_response`**, **`structured_output`**, and **`structured_output_failed`**. They are **not** included in the **`/chat/completions`** JSON; use **`extra_body`** when your gateway needs the same labels on the wire. **`extra_body`** rejects a bare **`llm_tags`** key so it cannot silently duplicate or collide with the audit field.

```python
out = ctx.llm.with_settings(llm_tags=["prompt:v2", "split:eval"]).parse(
    Answer, messages=[{"role": "user", "content": "Return {\"value\": 1}"}]
)
```

```bash
jq 'select(.type=="llm_response" and (.payload.llm_tags // [] | any(. == "prompt:v2")))' run.jsonl
```
...
Use **`ctx.llm.with_settings(response_format={"type": "json_object"})`** (or another OpenAI-compatible shape) when you want **`ctx.llm.complete_text`** to send **`response_format`** on **`/v1/chat/completions`**. Defaults apply until you clear them with **`with_settings(response_format={})`**. For a single call, **`complete_text(..., response_format=None)`** omits the field even when the bridge still carries a default.

JSONL **`effective`** carries a **small** audit copy: **`type`**, and for native-style **`json_schema`** traffic also **`json_schema_name`** / **`json_schema_strict`**, not the full embedded schema (that stays in the HTTP body and in **`schema_sha256`** on **`parse`** flows).

```python
text = ctx.llm.with_settings(response_format={"type": "json_object"}).complete_text(
    messages=[{"role": "user", "content": "Return one JSON object with key answer."}],
)
```

```bash
jq 'select(.type=="llm_request" and .payload.effective.response_format.type=="json_object")' run.jsonl
```

There is **no** first-class **`replayt inspect --response-format-type`** filter; use **`jq`** or DuckDB on **`payload.effective.response_format`** (see **Pattern: DuckDB ad-hoc analytics** in [`docs/EXAMPLES_PATTERNS.md`](../../docs/EXAMPLES_PATTERNS.md)).
