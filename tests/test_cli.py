...
def _reset_project_config_cache() -> None:
    ...
    cfg_mod._PROJECT_CONFIG_WALK = None
...
""".strip(),
    ...
    assert f"replayt bundle-export {run_id}" in md_out
    assert f"{run_id}-stakeholder-bundle.tar.gz" in md_out
...
def test_cli_inspect_output_markdown_support_style(tmp_path: Path) -> None:
    workflow_path = tmp_path / "inspect_sup_md.py"
    workflow_path.write_text(
        """
from replayt.workflow import Workflow

wf = Workflow("inspect_sup_md", version="1")
wf.set_initial("start")
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["inspect", run_id, "--log-dir", str(tmp_path), "--output", "markdown", "--style", "support"],
    )
    assert r.exit_code == 0
    out = r.stdout
    assert f"## Support handoff — `{run_id}`" in out
    assert "Support-oriented report (Markdown):" in out
    assert f"replayt report {run_id} --format markdown --style support" in out
    assert f"replayt replay {run_id} --format html --style support --out run.html" in out
    assert f"{run_id}-support-bundle.tar.gz" in out


def test_cli_inspect_style_support_rejects_text_output(tmp_path: Path) -> None:
    workflow_path = tmp_path / "inspect_style_txt.py"
    workflow_path.write_text(
        """
from replayt.workflow import Workflow

wf = Workflow("inspect_style_txt", version="1")
wf.set_initial("start")
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["inspect", run_id, "--log-dir", str(tmp_path), "--style", "support"],
    )
    assert r.exit_code != 0
    assert "support only applies with --output markdown" in ((r.stdout or "") + (r.stderr or ""))
...
def test_cli_inspect_unknown_run_id_shows_runs_hint(tmp_path: Path) -> None:
    ...
    assert "same log store" in out
    assert "REPLAYT_LOG_DIR" in out
    assert "log subdirectory" in out
...
def test_cli_run_missing_target_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "replayt try --list" in out
    assert "replayt_examples.e01_hello_world:wf" in out
...
def test_cli_init_list_text() -> None:
    ...
    assert "replayt run workflow.py --inputs-file inputs.example.json" in r.stdout
...
def test_cli_init_list_json() -> None:
    ...
    assert basic["cli"]["doctor_target"] == "replayt doctor --skip-connectivity --target workflow.py"
    assert basic["cli"]["validate_explicit"] == "replayt validate workflow.py --inputs-file inputs.example.json"
    assert basic["cli"]["dry_check_explicit"] == (
        "replayt run workflow.py --dry-check --inputs-file inputs.example.json"
    )
    assert basic["cli"]["run_explicit"] == "replayt run workflow.py --inputs-file inputs.example.json"
    assert basic["cli"]["ci_dry_check_explicit"] == (
        "replayt ci workflow.py --dry-check --strict-graph --inputs-file inputs.example.json"
    )
    assert basic["cli"]["dry_check"] == "replayt run --dry-check"
    assert basic["cli"]["run"] == "replayt run"
    assert "dry_run" not in basic["cli"]
    triage = next(t for t in data["templates"] if t["key"] == "issue-triage")
    assert triage["cli"]["dry_run"] == "replayt run --dry-run"
    assert triage["cli"]["run_explicit"] == "replayt run workflow.py --inputs-file inputs.example.json"
    yaml_t = next(t for t in data["templates"] if t["key"] == "yaml")
    assert yaml_t["cli"]["run_explicit"] == "replayt run workflow.yaml --inputs-file inputs.example.json"
...
def test_cli_run_rejects_invalid_run_id(tmp_path: Path) -> None:
    """Invalid --run-id must fail before run_hook builds REPLAYT_RUN_JSONL (path safety)."""
    runner = CliRunner()
    bad = "../evil"
    r = runner.invoke(
        app,
...
def test_cli_report_rejects_invalid_run_id(tmp_path: Path) -> None:
    runner = CliRunner()
    bad = "../evil"
    r = runner.invoke(app, ["report", bad, "--log-dir", str(tmp_path)])
    assert r.exit_code == 1
    assert "run_id must be" in r.stderr or "run_id must be" in r.stdout


def test_cli_export_run_rejects_invalid_run_id(tmp_path: Path) -> None:
    runner = CliRunner()
    bad = "../../x"
    out_tar = tmp_path / "out.tar.gz"
    r = runner.invoke(
        app,
        ["export-run", bad, "--out", str(out_tar), "--log-dir", str(tmp_path)],
    )
    assert r.exit_code == 1
    assert "run_id must be" in r.stderr or "run_id must be" in r.stdout


def test_cli_report_diff_rejects_invalid_run_id(tmp_path: Path) -> None:
    runner = CliRunner()
    bad = "bad/run"
    r = runner.invoke(
        app,
        ["report-diff", bad, "other", "--log-dir", str(tmp_path)],
    )
    assert r.exit_code == 1
    assert "run_id must be" in r.stderr or "run_id must be" in r.stdout


def test_cli_bundle_export_rejects_invalid_run_id(tmp_path: Path) -> None:
    runner = CliRunner()
    bad = ".."
    out_tar = tmp_path / "bundle.tar.gz"
    r = runner.invoke(
        app,
        ["bundle-export", bad, "--out", str(out_tar), "--log-dir", str(tmp_path)],
    )
    assert r.exit_code == 1
    assert "run_id must be" in r.stderr or "run_id must be" in r.stdout


def test_cli_inspect_invalid_run_id_includes_copy_hint(tmp_path: Path) -> None:
    runner = CliRunner()
    r = runner.invoke(app, ["inspect", "x/y", "--log-dir", str(tmp_path)])
    assert r.exit_code == 1
    err = (r.stderr or "") + (r.stdout or "")
    assert "run_id must be" in err
    assert "replayt runs" in err
...
""".strip(),
    ...
    assert "Stakeholder CLI handoff" in report.stdout
    assert "replayt bundle-export" in report.stdout
    assert "stakeholder-bundle.tar.gz" in report.stdout
    assert "replayt inspect" in report.stdout
    assert "--output markdown --style stakeholder" in report.stdout
...
""".strip(),
    ...
    assert "Stakeholder CLI handoff" in html_out.stdout
...
def test_cli_read_commands_do_not_create_missing_log_dir(tmp_path: Path) -> None:
    ...
    assert "e01_hello_world" in runs.stderr
    assert "replayt try --list" in runs.stderr
    assert stats.exit_code == 0
    assert json.loads(stats.stdout)["runs_total_on_disk"] == 0
    assert not missing_log_dir.exists()


def test_cli_runs_empty_after_status_filter_prints_filter_hint(tmp_path: Path) -> None:
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    rid = "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
    jsonl = log_dir / f"{rid}.jsonl"
    jsonl.write_text(
        json.dumps(
            {
                "seq": 1,
    ...
    assert "none matched your filters" in result.stderr
    assert "e01_hello_world" not in result.stderr
...
def test_init_gitignore_creates_file(tmp_path: Path) -> None:
    from replayt.cli.constants import INIT_GITIGNORE_LINES

    runner = CliRunner()
    result = runner.invoke(app, ["init-gitignore", "--path", str(tmp_path)])
    assert result.exit_code == 0
    gi = tmp_path / ".gitignore"
    assert gi.is_file()
    text = gi.read_text(encoding="utf-8")
    for line in INIT_GITIGNORE_LINES:
        assert line in text.splitlines()
    assert "Wrote" in result.stdout


def test_init_gitignore_merges_existing(tmp_path: Path) -> None:
    gi = tmp_path / ".gitignore"
...
def test_init_env_example_creates_file(tmp_path: Path) -> None:
    from replayt.cli.constants import INIT_ENV_EXAMPLE

    runner = CliRunner()
    result = runner.invoke(app, ["init-env-example", "--path", str(tmp_path)])
    assert result.exit_code == 0
    env_p = tmp_path / ".env.example"
    assert env_p.is_file()
    text = env_p.read_text(encoding="utf-8")
    assert text == INIT_ENV_EXAMPLE
    assert "REPLAYT_TARGET" in text
    assert "Wrote" in result.stdout


def test_init_env_example_refuses_overwrite_without_force(tmp_path: Path) -> None:
    env_p = tmp_path / ".env.example"
    env_p.write_text("custom=1\n", encoding="utf-8")
    runner = CliRunner()
    result = runner.invoke(app, ["init-env-example", "--path", str(tmp_path)])
    assert result.exit_code == 1
    assert env_p.read_text(encoding="utf-8") == "custom=1\n"
    assert "Refusing" in result.stderr


def test_init_env_example_force_overwrites(tmp_path: Path) -> None:
    from replayt.cli.constants import INIT_ENV_EXAMPLE

    env_p = tmp_path / ".env.example"
    env_p.write_text("custom=1\n", encoding="utf-8")
    runner = CliRunner()
    result = runner.invoke(app, ["init-env-example", "--path", str(tmp_path), "--force"])
    assert result.exit_code == 0
    assert env_p.read_text(encoding="utf-8") == INIT_ENV_EXAMPLE
...
""".strip(),
    ...
    assert f"replayt inspect {id_a} --output markdown --style stakeholder" in handoff.stdout
...
def test_cli_run_module_target_empty_module_or_var_onboarding(tmp_path: Path) -> None:
    runner = CliRunner()
    r = runner.invoke(app, ["run", ":wf", "--dry-run", "--log-dir", str(tmp_path)])
    assert r.exit_code != 0
    out = (r.stdout or "") + (r.stderr or "")
    assert "MODULE:VAR" in out
    assert "replayt_examples.e01_hello_world:wf" in out.replace("\n", " ")

    r2 = runner.invoke(
        app,
        ["run", "replayt_examples.e01_hello_world:", "--dry-run", "--log-dir", str(tmp_path)],
    )
    assert r2.exit_code != 0
    out2 = (r2.stdout or "") + (r2.stderr or "")
    assert "MODULE:VAR" in out2


def test_cli_run_module_target_extra_colon_onboarding(tmp_path: Path) -> None:
    runner = CliRunner()
    r = runner.invoke(
        app,
        [
            "run",
            "replayt_examples.e01_hello_world:wf:extra",
            "--dry-run",
            "--log-dir",
            str(tmp_path),
        ],
    ...
    assert r.exit_code != 0
    out = (r.stdout or "") + (r.stderr or "")
    assert "colon" in out.lower()
    assert "replayt_examples.e01_hello_world:wf" in out.replace("\n", " ")


def test_cli_run_module_target_wrong_attr_lists_exports(tmp_path: Path) -> None:
    runner = CliRunner()
    r = runner.invoke(
        app,
        [
            "run",
            "replayt_examples.e01_hello_world:workflow",
            "--dry-run",
            "--log-dir",
            str(tmp_path),
        ],
    ...
    assert r.exit_code != 0
    out = (r.stdout or "") + (r.stderr or "")
    assert "no attribute 'workflow'" in out
    assert "`wf`" in out or "wf" in out
    assert "replayt run replayt_examples.e01_hello_world:wf" in out.replace("\n", " ")
    assert "doctor --skip-connectivity --target replayt_examples.e01_hello_world:wf" in out.replace("\n", " ")
...
def test_cli_export_hook_receives_policy_context(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    script = (
        ...
        "'runtime': json.loads(os.environ['REPLAYT_RUN_STARTED_RUNTIME_JSON']), "
        "'initial_state': os.environ.get('REPLAYT_RUN_STARTED_INITIAL_STATE'), "
    ...
    assert data["workflow_meta"] is None
    assert data["inputs"] == {"customer_name": "Sam"}
    assert data["policy_hook_name"] == "export_hook"
    assert data["initial_state"] == "greet"
    assert data["runtime"]["engine"]["log_mode"] == "redacted"
...
def test_cli_runs_output_markdown_table_and_handoff(tmp_path: Path) -> None:
    log_dir = tmp_path / "runs"
    log_dir.mkdir()
    lines = [
        {
            ...
            "seq": 1,
            "type": "run_started",
            "payload": {"workflow_name": "demo", "workflow_version": "2"},
        },
        {
    ...
    (log_dir / "rid-md.jsonl").write_text("\n".join(json.dumps(x) for x in lines) + "\n", encoding="utf-8")

    runner = CliRunner()
    result = runner.invoke(app, ["runs", "--log-dir", str(log_dir), "--output", "markdown"])
    assert result.exit_code == 0
    assert "## Recent replayt runs" in result.stdout
    assert "| rid-md |" in result.stdout
    assert "demo@2" in result.stdout
    assert "## Stakeholder CLI handoff" in result.stdout
    assert "replayt inspect RUN_ID --output markdown --style stakeholder" in result.stdout
    assert "replayt bundle-export RUN_ID --out RUN_ID-stakeholder-bundle.tar.gz" in result.stdout


def test_cli_runs_output_markdown_support_style_title(tmp_path: Path) -> None:
    log_dir = tmp_path / "runs"
    log_dir.mkdir()
    lines = [
        {
    ...
    (log_dir / "rid-s.jsonl").write_text("\n".join(json.dumps(x) for x in lines) + "\n", encoding="utf-8")

    runner = CliRunner()
    result = runner.invoke(
        app,
        ["runs", "--log-dir", str(log_dir), "--output", "markdown", "--style", "support"],
    )
    assert result.exit_code == 0
    assert "## Support triage — run inventory" in result.stdout
    assert "replayt inspect RUN_ID --output markdown --style support" in result.stdout
    assert "RUN_ID-support-bundle.tar.gz" in result.stdout


def test_cli_runs_style_support_rejected_without_markdown_output(tmp_path: Path) -> None:
    log_dir = tmp_path / "runs"
    log_dir.mkdir()
    runner = CliRunner()
    result = runner.invoke(app, ["runs", "--log-dir", str(log_dir), "--style", "support"])
    assert result.exit_code != 0
    assert "support only applies" in result.stderr.lower()


def test_cli_runs_output_markdown_empty_store_stderr_hints(tmp_path: Path) -> None:
    log_dir = tmp_path / "no_runs"
    log_dir.mkdir()
    runner = CliRunner()
    result = runner.invoke(app, ["runs", "--log-dir", str(log_dir), "--output", "markdown"])
    assert result.exit_code == 0
    assert "No runs matched this query" in result.stdout
    assert "## Stakeholder CLI handoff" in result.stdout
...
def test_cli_runs_structured_schema_filter_matches_llm_schema_name(tmp_path: Path) -> None:
    log_dir = tmp_path / "logs"
    log_dir.mkdir()
    lines = [
        {
    ...
    (log_dir / "rid-llm-tag.jsonl").write_text(
        "\n".join(json.dumps(x) for x in lines) + "\n", encoding="utf-8"
    )

    runner = CliRunner()
    r_runs = runner.invoke(app, ["runs", "--log-dir", str(log_dir), "--structured-schema", "DraftRaw"])
    assert r_runs.exit_code == 0
    assert "rid-llm-tag" in r_runs.stdout

    r_inspect = runner.invoke(
        app,
        [
            "inspect",
            "rid-llm-tag",
    ...
    insp = json.loads(r_inspect.stdout)
    assert [e["type"] for e in insp["events"]] == ["llm_request", "llm_response"]
...
def test_cli_report_diff_includes_context_and_failure_sections(tmp_path: Path) -> None:
    ...
    assert "Stakeholder CLI handoff" in st.stdout
    assert "replayt diff ctx-a ctx-b --output json" in st.stdout
    assert "replayt report-diff ctx-a ctx-b --format markdown --style stakeholder" in st.stdout
    assert "replayt bundle-export ctx-a" in st.stdout
    assert "replayt bundle-export ctx-b" in st.stdout
    assert "ctx-a-stakeholder-bundle.tar.gz" in st.stdout
    assert "ctx-b-stakeholder-bundle.tar.gz" in st.stdout
    assert "replayt inspect ctx-a --output markdown --style stakeholder" in st.stdout
    assert "Inspect run A (JSON for engineers)" in st.stdout

    assert "Stakeholder CLI handoff" in sup.stdout
    assert "replayt diff ctx-a ctx-b --output json" in sup.stdout
    assert "replayt inspect ctx-a --output markdown --style support" in sup.stdout
    assert "replayt report ctx-a --format markdown --style support" in sup.stdout
    assert "replayt bundle-export ctx-a --out ctx-a-support-bundle.tar.gz --report-style support" in sup.stdout
    assert "ctx-b-support-bundle.tar.gz" in sup.stdout

    default_diff = runner.invoke(
        app,
        ["report-diff", "ctx-a", "ctx-b", "--log-dir", str(log_dir), "--style", "default"],
    )
    assert default_diff.exit_code == 0
    assert "Stakeholder CLI handoff" not in default_diff.stdout
...
def test_cli_report_diff_markdown_writes_context_and_failure_sections(tmp_path: Path) -> None:
    ...
    assert "## Stakeholder CLI handoff" in md_st.stdout
    assert "replayt diff ctx-a ctx-b --output json" in md_st.stdout
...
def test_cli_ci_writes_summary_json(tmp_path: Path) -> None:
    ...
    assert data["python_cache_tag"] == getattr(sys.implementation, "cache_tag", None)
    assert data["python_utf8_mode"] == int(sys.flags.utf8_mode)
    assert data["python_executable"] == sys.executable
    assert data["platform"] == sys.platform
    assert data["machine"] == platform.machine()
    assert data["os_cpu_count"] == os.cpu_count()
    assert data["filesystem_encoding"] == (sys.getfilesystemencoding() or "")
    assert data["stdout_encoding"] == getattr(sys.stdout, "encoding", None)
    if sys.platform == "win32":
        assert data["ulimit_nofile_soft"] is None
        assert data["ulimit_nofile_hard"] is None
    else:
        import resource
...
def test_ci_run_summary_runtime_fields_ci_markers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert fields["python_cache_tag"] == getattr(sys.implementation, "cache_tag", None)
    assert fields["python_utf8_mode"] == int(sys.flags.utf8_mode)
    assert fields["os_cpu_count"] == os.cpu_count()
...
def test_cli_resume_hook_receives_workflow_contract_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    module_path = tmp_path / "resume_hook_policy_wf.py"
    module_path.write_text(
        """
from replayt.workflow import Workflow

wf = Workflow("approval_flow", meta={"catalog_entry": "ce-42", "llm_defaults": {"model": "x"}})
wf.set_initial("gate")
wf.note_transition("gate", "done")
...
""".strip(),
    ...
    script = (
        ...
        "'log_dir': os.environ.get('REPLAYT_LOG_DIR'), "
        "'run_jsonl': os.environ.get('REPLAYT_RUN_JSONL'), "
    ...
    resume = runner.invoke(
        app,
        ["resume", "resume_hook_policy_wf:wf", run_id, "--approval", "ship", "--log-dir", str(tmp_path)],
    )
    assert resume.exit_code == 0
    data = json.loads(hook_out.read_text(encoding="utf-8"))
    assert data["approval_id"] == "ship"
    assert data["reject"] == "0"
    exp = load_target("resume_hook_policy_wf:wf").contract()
    ...
    assert data["workflow_meta"] == {"catalog_entry": "ce-42"}
    assert data["inputs"] == {"case_ref": "INC-77"}
    assert data["runtime"]["engine"]["log_mode"] == "redacted"
    assert data["log_mode"] == "redacted"
    assert data["forbid_full"] == "1"
    assert data["redact_keys"] == ["patient_id"]
    assert Path(data["log_dir"]).resolve() == Path(tmp_path).resolve()
    assert Path(data["run_jsonl"]).resolve() == (Path(tmp_path) / f"{run_id}.jsonl").resolve()
    import resume_hook_policy_wf as resume_hook_policy_wf_mod
    ...
    assert data["initial_state"] == "gate"
...
def test_cli_run_hook_receives_policy_context(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    script = (
        ...
        "'run_started_ts': os.environ.get('REPLAYT_RUN_STARTED_TS'), "
        "'runtime': (json.loads(os.environ['REPLAYT_RUN_STARTED_RUNTIME_JSON']) "
        "if os.environ.get('REPLAYT_RUN_STARTED_RUNTIME_JSON') else None), "
        "'initial_state': os.environ.get('REPLAYT_RUN_STARTED_INITIAL_STATE'), "
        "'policy_hook_name': os.environ.get('REPLAYT_POLICY_HOOK_NAME'), "
    ...
    assert Path(data["run_jsonl"]).resolve() == (tmp_path / "logs" / f"{run_id}.jsonl").resolve()
    assert data["log_mode"] == "redacted"
    assert data["forbid_full"] == "1"
    ...
    assert data.get("runtime") is None
    assert data["initial_state"] == "greet"
    assert data["mode"] == "run"
    assert data["target"] == "replayt_examples.e01_hello_world:wf"
    assert data["dry_run"] == "1"
...
""".strip(),
    ...
    assert first_hook["initial_state"] == "gate"
    ...
    assert second_hook["initial_state"] == "gate"


def test_cli_run_hook_receives_policy_hook_context_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    hook_out = tmp_path / "policy_hook_ctx.json"
    script = (
        "import json, os; "
        "from pathlib import Path; "
        "Path(os.environ['HOOK_OUT']).write_text("
        "json.dumps(json.loads(os.environ['REPLAYT_POLICY_HOOK_CONTEXT_JSON'])), encoding='utf-8')"
    )
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text(
    ...
    monkeypatch.setenv(
        ...
        '{"pipeline_job":"ci-99","tier":"staging"}',
    )
    import replayt.cli.config as cfg_mod
    ...
    assert result.exit_code == 0
    recorded = json.loads(hook_out.read_text(encoding="utf-8"))
    assert recorded == {"pipeline_job": "ci-99", "tier": "staging"}


def test_cli_run_hook_policy_hook_context_cli_overrides_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    hook_out = tmp_path / "policy_hook_ctx_cli.json"
    script = (
        "import json, os; "
        "from pathlib import Path; "
        "Path(os.environ['HOOK_OUT']).write_text("
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("HOOK_OUT", str(hook_out))
    monkeypatch.setenv("REPLAYT_POLICY_HOOK_CONTEXT_JSON", '{"from_env":true}')
    import replayt.cli.config as cfg_mod
    ...
    assert result.exit_code == 0
    recorded = json.loads(hook_out.read_text(encoding="utf-8"))
    assert recorded == {"from_cli": 1}


def test_cli_run_hook_receives_workflow_meta_json(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    hook_out = tmp_path / "run_hook_wm.json"
    script = (
        "import json, os; "
        "from pathlib import Path; "
        "wm = os.environ.get('REPLAYT_WORKFLOW_META_JSON'); "
        "Path(os.environ['HOOK_OUT']).write_text("
        "json.dumps(None if wm is None else json.loads(wm)), encoding='utf-8')"
    )
    wf_path = tmp_path / "wm_hook.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
...
""".strip(),
    ...
    result = runner.invoke(
        app,
        ["run", str(wf_path), "--log-dir", str(tmp_path / "logs"), "--dry-run"],
    )
    assert result.exit_code == 0
    recorded = json.loads(hook_out.read_text(encoding="utf-8"))
    assert recorded == {"owning_team": "platform", "package_id": "acme-risk"}
    run_id = next(line.split("=", 1)[1] for line in result.stdout.splitlines() if line.startswith("run_id="))
    events = [
        json.loads(line)
        for line in (tmp_path / "logs" / f"{run_id}.jsonl").read_text(encoding="utf-8").splitlines()
    ...
    assert started["payload"]["workflow_meta"] == {"owning_team": "platform", "package_id": "acme-risk"}
...
def test_cli_validate_text_ok_prints_next_steps(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "v.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
...
""".strip(),
    ...
    out = r.stdout
    assert "OK:" in out
    assert "Next: replayt run" in out
    assert str(wf_path) in out
    assert "--dry-check" in out
    assert "Then: replayt run" in out
    assert "docs/QUICKSTART.md" in out


def test_cli_validate_text_ok_next_steps_repeat_strict_graph(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "v2.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("v2")
...
""".strip(),
    ...
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()
    r = runner.invoke(app, ["validate", str(wf_path), "--strict-graph"])
    assert r.exit_code == 0
    assert "Next: replayt run" in r.stdout
    assert "--dry-check" in r.stdout
    assert r.stdout.count("--strict-graph") >= 2


def test_cli_validate_print_inputs_template_ok(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "expects_wf.py"
    wf_path.write_text(
        """
from typing import Optional
from replayt.workflow import Workflow
wf = Workflow("expects_wf")
wf.set_initial("a")
@wf.step("a", expects={"title": str, "count": int, "flag": bool, "ratio": float, "tags": list, "meta": dict})
def a(ctx):
    return None
@wf.step("b", expects={"extra": str, "maybe": Optional[str]})
def b(ctx):
    return None
""".strip(),
        encoding="utf-8",
    ...
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()
    r = runner.invoke(app, ["validate", str(wf_path), "--print-inputs-template"])
    assert r.exit_code == 0
...
def test_cli_validate_print_inputs_template_conflict_null(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "conflict.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("c")
wf.set_initial("a")
wf.note_transition("a", "b")
@wf.step("a", expects={"x": str})
def a(ctx):
    return "b"
@wf.step("b", expects={"x": int})
def b(ctx):
    return None
""".strip(),
        encoding="utf-8",
    ...
    r = runner.invoke(app, ["validate", str(wf_path), "--print-inputs-template"])
    assert r.exit_code == 0
    assert json.loads(r.stdout.strip()) == {"x": None}


def test_cli_validate_print_run_one_liner_ok(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "expects_wf2.py"
    wf_path.write_text(
        """
from typing import Optional
from replayt.workflow import Workflow
wf = Workflow("expects_wf2")
wf.set_initial("a")
@wf.step("a", expects={"title": str, "count": int})
def a(ctx):
    return None
@wf.step("b", expects={"extra": str, "maybe": Optional[str]})
def b(ctx):
...
""".strip(),
    ...
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()
    r = runner.invoke(app, ["validate", str(wf_path), "--print-run-one-liner"])
    assert r.exit_code == 0
    line = r.stdout.strip()
    assert line.startswith("replayt run ")
    parts = shlex.split(line)
    assert parts[:2] == ["replayt", "run"]
    i = parts.index("--inputs-json")
    assert json.loads(parts[i + 1]) == {"count": 0, "extra": "", "maybe": "", "title": ""}
    assert "--strict-graph" not in parts
    assert "Tip:" in r.stderr


def test_cli_validate_print_run_one_liner_strict_graph(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "two_state.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("two")
...
""".strip(),
    ...
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()
    r = runner.invoke(app, ["validate", str(wf_path), "--strict-graph", "--print-run-one-liner"])
    assert r.exit_code == 0
    parts = shlex.split(r.stdout.strip())
    assert parts[-1] == "--strict-graph"


def test_cli_validate_print_run_one_liner_skipped_when_invalid(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "bad2.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("bad2")
wf.set_initial("missing")
@wf.step("a")
def a(ctx):
    return None
""".strip(),
...
def test_cli_validate_print_run_one_liner_rejects_json_format_combo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "v2.py"
    wf_path.write_text(
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["validate", str(wf_path), "--print-run-one-liner", "--format", "json"],
    )
    assert r.exit_code == 2
    assert "Cannot combine" in r.stderr


def test_cli_validate_print_ci_one_liner_ok(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "expects_wf_ci.py"
    wf_path.write_text(
        """
from typing import Optional
from replayt.workflow import Workflow
wf = Workflow("expects_wf_ci")
wf.set_initial("a")
@wf.step("a", expects={"title": str, "count": int})
def a(ctx):
    return None
...
""".strip(),
    ...
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()
    r = runner.invoke(app, ["validate", str(wf_path), "--print-ci-one-liner"])
    assert r.exit_code == 0
    line = r.stdout.strip()
    assert line.startswith("replayt ci ")
    parts = shlex.split(line)
    assert parts[:2] == ["replayt", "ci"]
    i = parts.index("--inputs-json")
    assert json.loads(parts[i + 1]) == {"count": 0, "extra": "", "maybe": "", "title": ""}
    assert "--strict-graph" not in parts
    assert "Tip:" in r.stderr
...
def test_cli_validate_print_ci_one_liner_strict_graph(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    wf_path = tmp_path / "two_state_ci.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("two_ci")
...
""".strip(),
    ...
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()
    r = runner.invoke(app, ["validate", str(wf_path), "--strict-graph", "--print-ci-one-liner"])
    assert r.exit_code == 0
    parts = shlex.split(r.stdout.strip())
    assert parts[-1] == "--strict-graph"


def test_cli_validate_print_ci_one_liner_skipped_when_invalid(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "bad_ci.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("bad_ci")
...
def test_cli_validate_print_ci_one_liner_rejects_json_format_combo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "v_ci.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("v_ci")
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["validate", str(wf_path), "--print-ci-one-liner", "--format", "json"],
    )
    assert r.exit_code == 2
    assert "Cannot combine" in r.stderr


def test_cli_validate_print_ci_one_liner_rejects_inputs_template_combo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "v3_ci.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("v3_ci")
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["validate", str(wf_path), "--print-ci-one-liner", "--print-inputs-template"],
    )
    assert r.exit_code == 2
    assert "Cannot combine" in r.stderr


def test_cli_validate_print_ci_one_liner_rejects_run_one_liner_combo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "v4_ci.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("v4_ci")
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["validate", str(wf_path), "--print-ci-one-liner", "--print-run-one-liner"],
    )
    assert r.exit_code == 2
    assert "Cannot combine" in r.stderr


def test_cli_validate_print_run_one_liner_rejects_inputs_template_combo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "v3.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("v3")
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["validate", str(wf_path), "--print-run-one-liner", "--print-inputs-template"],
    )
    assert r.exit_code == 2
    assert "Cannot combine" in r.stderr


def test_cli_validate_inputs_template_rejects_json_format_combo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "v.py"
    wf_path.write_text(
...
""".strip(),
    ...
    r = runner.invoke(
        app,
        ["validate", str(wf_path), "--print-inputs-template", "--format", "json"],
    )
    assert r.exit_code == 2
    assert "Cannot combine" in r.stderr


def test_cli_validate_inputs_template_skipped_when_invalid(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf_path = tmp_path / "bad.py"
    wf_path.write_text(
        """
from replayt.workflow import Workflow
wf = Workflow("bad")
...
def test_cli_run_target_non_workflow_file_rejected(
    ...
    assert result.exit_code == 2
    text = _strip_ansi(result.stdout + result.stderr)
    assert "exists on disk" in text.lower()
    assert ".py" in text and "workflow file" in text.lower()


def test_cli_run_target_json_file_suggests_inputs_file_flag(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_cli_doctor_format_json_healthy(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    egress = data["egress_trust_env"]
    assert isinstance(egress, list) and egress
    assert egress == sorted(egress, key=lambda r: r["name"])
    names_eg = [r["name"] for r in egress]
    assert len(names_eg) == len(set(names_eg))
...
def test_cli_config_json_includes_llm_egress_trust_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert r.exit_code == 0
    data = json.loads(r.stdout)
    et = data["llm"]["egress_trust_env"]
    assert isinstance(et, list) and et
    assert et == sorted(et, key=lambda r: r["name"])
    http = next(row for row in et if row["name"] == "HTTP_PROXY")
    assert http["present"] is False
...
def test_cli_version_format_json() -> None:
    ...
    assert data["replayt_version_tuple"] == list(rt.__version_tuple__)
    assert data["platform"] == sys.platform
    ...
    from replayt.cli.skill_loop_env import build_skill_loop_env_contract
    from replayt.cli.skill_loop_placeholders import build_skill_loop_placeholder_contract

    assert data["skill_loop_env_contract"] == build_skill_loop_env_contract()
    assert data["skill_loop_placeholder_contract"] == build_skill_loop_placeholder_contract()
    py = data["python"]
    assert py["version"] == f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    impl = py["implementation"]
    assert impl["name"] == sys.implementation.name
    assert impl["version"] == ".".join(str(part) for part in sys.implementation.version)
    assert impl["cache_tag"] == getattr(sys.implementation, "cache_tag", None)
    import platform as platform_mod

    assert data["platform_machine"] == platform_mod.machine()
    ...
    assert schemas["log_mode_contract"] == "replayt.log_mode_contract.v1"
    assert schemas["jsonl_event_types_contract"] == "replayt.jsonl_event_types_contract.v1"
    ...
    assert data["log_mode_contract"] == build_log_mode_contract()
    assert data["jsonl_event_types_contract"] == build_jsonl_event_types_contract()
    ...
    for sc, routes in cjson["subcommands"].items():
        prefix = sc.replace("-", "_")
        for row in routes:
            assert row["schema"] == schemas[row["schema_key"]]
            rid = row["route_id"]
            assert rid not in seen_route_ids
            seen_route_ids.add(rid)
            assert rid.startswith(prefix + "."), (sc, rid)
            ec = row["exit_codes_with_json_on_stdout"]
            assert ec == sorted(set(ec))
            assert all(isinstance(x, int) for x in ec)
            pr = row["positional_required"]
            po = row["positional_optional"]
            assert isinstance(pr, list) and isinstance(po, list)
            assert all(isinstance(x, str) and x for x in pr)
            assert all(isinstance(x, str) and x for x in po)
            allowed_slots = {"TARGET", "RUN_ID", "RUN_A", "RUN_B"}
            assert set(pr) <= allowed_slots and set(po) <= allowed_slots
            assert not (set(pr) & set(po))
    ...
    assert cjson["subcommands"]["inspect"][0]["positional_required"] == ["RUN_ID"]
    assert cjson["subcommands"]["inspect"][0]["positional_optional"] == []
    assert cjson["subcommands"]["diff"][0]["positional_required"] == ["RUN_A", "RUN_B"]
    assert cjson["subcommands"]["run"][1]["positional_optional"] == ["TARGET"]
    assert cjson["subcommands"]["version"][0]["positional_required"] == []
    assert cjson["subcommands"]["version"][0]["positional_optional"] == []
    ...
    tpf = cjson["trust_profile_machine_flags"]
    assert tpf["workflow_run_execute"]["appends_run_timeline"] is True
    assert tpf["inventory_read_only"]["appends_run_timeline"] is False
    assert tpf["doctor_preflight"]["may_probe_default_llm_http"] is True
    assert tpf["try_copy_scaffold"]["writes_scaffold_paths"] is True
    stream = cjson["subprocess_stream_semantics"]
    assert stream["stdout"]["encoding"] == "utf-8"
    assert "Exactly one JSON value" in stream["stdout"]["when_json_route_active"]
    assert stream["stderr"]["may_contain_human_or_progress_hints_when_stdout_is_json"] is True
    assert stream["stderr"]["documented_cases"][0]["subcommand"] == "ci"
    assert "wrapper_note" in stream
    typer_phase = cjson["typer_pre_dispatch_phase"]
    assert typer_phase["typical_exit_code"] == 2
    assert "Typer may exit" in typer_phase["summary"]
    overlap = typer_phase["exit_code_overlap"]
    assert "replayt.run_result.v1" in overlap["disambiguation_for_json_stdout_routes"]
    assert "status paused" in overlap["disambiguation_for_json_stdout_routes"]
    assert "replayt resume" in overlap["disambiguation_for_resume_text_stdout"]
    tpre = data["cli_exit_codes"]["typer_pre_dispatch_failures"]
    assert tpre["typical_exit_code"] == 2
    assert set(tpre["overlaps_workflow_pause_exit_code_on"]) == {"ci", "resume", "run", "try"}
    ...
    for hook_key, hook_data in hooks.items():
        ...
        assert "REPLAYT_RUN_STARTED_RUNTIME_JSON" in hook_data["injected_env_vars"]
        assert "REPLAYT_RUN_STARTED_INITIAL_STATE" in hook_data["injected_env_vars"]
    ...
    assert "REPLAYT_RUN_JSONL" in rh["injected_env_vars"]
    assert "REPLAYT_WORKFLOW_ENTRY_PATH" in rh["injected_env_vars"]
    ...
    assert "REPLAYT_WORKFLOW_ENTRY_PATH" in eh["injected_env_vars"]
    ...
    assert "REPLAYT_RUN_INPUTS_JSON" in eh["injected_env_vars"]
    sh = hooks["seal_hook"]
    ...
    assert "REPLAYT_RUN_INPUTS_JSON" in sh["injected_env_vars"]
    res_h = hooks["resume_hook"]
    assert "REPLAYT_LOG_DIR" in res_h["injected_env_vars"]
    assert "REPLAYT_RUN_JSONL" in res_h["injected_env_vars"]
    assert "REPLAYT_WORKFLOW_ENTRY_PATH" in res_h["injected_env_vars"]
    ...
    assert "REPLAYT_RUN_INPUTS_JSON" in res_h["injected_env_vars"]
    ...
    assert "REPLAYT_RUN_INPUTS_JSON" in vh["injected_env_vars"]
    ...
    assert data["project_config_resolution"] == build_project_config_resolution_report()
    assert data["cli_machine_readable_schemas"]["project_config_resolution"] == "replayt.project_config_resolution.v1"
    assert data["cli_run_defaults_contract"] == build_cli_run_defaults_contract()
    assert data["cli_machine_readable_schemas"]["cli_run_defaults_contract"] == "replayt.cli_run_defaults_contract.v1"
    assert data["project_setting_precedence_contract"] == build_project_setting_precedence_contract()
    assert (
        data["cli_machine_readable_schemas"]["project_setting_precedence_contract"]
        == "replayt.project_setting_precedence_contract.v1"
    )


def test_project_config_resolution_pyproject_winner(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _reset_project_config_cache()
    root = tmp_path
    (root / "pyproject.toml").write_text('[tool.replayt]\nlog_dir = "logs"\n', encoding="utf-8")
    monkeypatch.chdir(root)
    try:
        from replayt.cli.config import build_project_config_resolution_report

        rep = build_project_config_resolution_report()
        assert rep["schema"] == "replayt.project_config_resolution.v1"
        assert rep["cwd"] == str(root.resolve())
        assert rep["winner"] is not None
        assert rep["winner"]["kind"] == "pyproject"
        assert rep["winner"]["path"] == str((root / "pyproject.toml").resolve())
        assert len(rep["walk"]) == 1
        assert rep["walk"][0]["stopped_here"] is True
        assert rep["walk"][0]["pyproject_has_tool_replayt"] is True
    finally:
        _reset_project_config_cache()


def test_project_config_resolution_replaytrc_shadows_pyproject(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _reset_project_config_cache()
    root = tmp_path
    (root / "pyproject.toml").write_text('[tool.replayt]\ntarget = "pkg:wf"\n', encoding="utf-8")
    (root / ".replaytrc.toml").write_text('target = "other:wf"\n', encoding="utf-8")
    monkeypatch.chdir(root)
    try:
        from replayt.cli.config import build_project_config_resolution_report
        ...
        rep = build_project_config_resolution_report()
        assert rep["winner"] is not None
        assert rep["winner"]["kind"] == "replaytrc"
        assert rep["shadowed_sources"] == [str((root / "pyproject.toml").resolve())]
        assert rep["walk"][0]["stopped_here"] is True
        assert rep["walk"][0]["pyproject_has_tool_replayt"] is True
    finally:
        _reset_project_config_cache()
...
def test_project_config_resolution_nested_cwd(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _reset_project_config_cache()
    root = tmp_path
    sub = root / "sub"
    sub.mkdir()
    ...
    try:
        ...
        rep = build_project_config_resolution_report()
        assert len(rep["walk"]) == 2
        assert rep["walk"][0]["stopped_here"] is False
        assert rep["walk"][1]["stopped_here"] is True
        assert rep["winner"] is not None
        assert rep["winner"]["kind"] == "pyproject"
    finally:
        _reset_project_config_cache()
...
def test_cli_run_defaults_contract_stable_precedence() -> None:
    ...
    ri = doc["run_inputs"]
    assert set(ri["commands_using_resolve_run_inputs_json"]) == {"ci", "doctor", "run", "validate"}
    ip = ri["precedence"]
...
def test_log_mode_contract_stable_shape() -> None:
    from replayt.cli.run_support import LOG_MODE_CONTRACT_SCHEMA, build_log_mode_contract

    doc = build_log_mode_contract()
    assert doc["schema"] == LOG_MODE_CONTRACT_SCHEMA
    assert doc["payload"]["python_enum"] == "replayt.types.LogMode"
    vals = [m["value"] for m in doc["modes"]]
    assert vals == ["full", "redacted", "structured_only"]
    assert doc["cli_flag"] == "--log-mode"
    assert doc["project_config_key"] == "log_mode"
    assert doc["policy_hook_env_var"] == "REPLAYT_LOG_MODE"
    assert doc["project_setting_precedence_cross_reference"] == "log_mode"


def test_jsonl_event_types_contract_stable_shape() -> None:
    from replayt.cli.run_support import (
    ...
    doc = build_jsonl_event_types_contract()
    assert doc["schema"] == JSONL_EVENT_TYPES_CONTRACT_SCHEMA
    assert doc["payload"]["envelope_type_field"] == "type"
    assert doc["log_mode_contract_cross_reference"] == LOG_MODE_CONTRACT_SCHEMA
    vals = [e["value"] for e in doc["event_types"]]
...
def test_project_setting_precedence_contract_stable_shape() -> None:
    ...
    doc = build_project_setting_precedence_contract()
    assert doc["schema"] == "replayt.project_setting_precedence_contract.v1"
    assert doc["extends_cli_run_defaults_contract"] == CLI_RUN_DEFAULTS_CONTRACT_SCHEMA
    ids = [s["id"] for s in doc["settings"]]
    assert ids == sorted(ids)
    hook_setting = next(s for s in doc["settings"] if s["id"] == "trusted_policy_hook_argv")
    assert hook_setting["merge_policy"] == "replace_not_concatenate"
    hook_keys = [h["config_key"] for h in hook_setting["hooks"]]
    assert hook_keys == sorted(hook_keys)
    assert hook_keys == [
        "export_hook",
        "resume_hook",
        "run_hook",
    ...
    timeout_setting = next(s for s in doc["settings"] if s["id"] == "trusted_policy_hook_timeouts")
    assert [h["config_key"] for h in timeout_setting["hooks"]] == hook_keys
...
def test_run_started_hook_json_blobs_from_jsonl_path_finds_first_after_prefix(tmp_path: Path) -> None:
    ...
    log.write_text(
        ...
        '"workflow_meta":{"package_id":"pkg-1"}'
        "}}\n"
        '{"not": "valid json"}\n',
        encoding="utf-8",
    )
    meta_j, tags_j, exp_j, wf_meta_j = run_started_hook_json_blobs_from_jsonl_path(log)
    ...
    assert wf_meta_j is not None and json.loads(wf_meta_j) == {"package_id": "pkg-1"}
...
def test_run_started_hook_json_blobs_from_jsonl_path_invalid_utf8_returns_none(tmp_path: Path) -> None:
    ...
    assert run_started_hook_json_blobs_from_jsonl_path(log) == (None, None, None, None)
...
def test_cli_version_operational_paths_respects_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        assert op["ci_artifact_readiness_ok"] is True
        names = {row["name"] for row in op["ci_artifact_readiness"]}
        assert names == {"ci_github_summary_ready", "ci_junit_xml_ready", "ci_summary_json_ready"}
        assert all(row["ok"] for row in op["ci_artifact_readiness"])
    finally:
        _reset_project_config_cache()


def test_cli_version_operational_paths_ci_artifact_readiness_detects_bad_junit_path(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _reset_project_config_cache()
    monkeypatch.chdir(tmp_path)
    ...
    bad_dir.mkdir()
    monkeypatch.setenv("REPLAYT_JUNIT_XML", str(bad_dir))
    try:
        runner = CliRunner()
        r = runner.invoke(app, ["version", "--format", "json"])
        assert r.exit_code == 0
        data = json.loads(r.stdout)
        op = data["operational_paths"]
        assert op["ci_artifact_readiness_ok"] is False
        junit_row = next(x for x in op["ci_artifact_readiness"] if x["name"] == "ci_junit_xml_ready")
        assert junit_row["ok"] is False
        assert "directory" in junit_row["detail"].lower()
    finally:
        _reset_project_config_cache()
